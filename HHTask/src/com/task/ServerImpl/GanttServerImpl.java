...
public class GanttServerImpl implements GanttServer{
	...
	public boolean addOneGantt(Gantt gantt) {
		gantt.setBelongLayer(1);
		totalDao.save(gantt);
		gantt.setRootId(gantt.getId()); 
	...
	public List<Gantt> queryAllGantts() {
		...
		List<Gantt> list = new ArrayList<Gantt>();
		for(Gantt g:gantts){
			//直接孩子节点数量
			list.add(g);
			List<Gantt> child=totalDao.find("from Gantt where  id <> "+g.getId()+" and rootId="+g.getId());
			list.addAll(child);
...
//			g.setAllParentInt(gainAllChild(g.getId(),null));//递归查找父节点以及父节点的父节点Integer
	...
	//查询所有甘特项
	public List<Gantt> queryAllGantts1(Integer rootId) {
		String str = "";
		if(rootId!=null){
			str = " and rootId = "+rootId;
		}
		List<Gantt> gantts=totalDao.find("from Gantt where 1=1 "+str);
		return gantts;
	}
	//删除甘特一项
...
//			g.setChildLength(childLength);
	...
	@Override
	public String addSon(Gantt gantt) {
		if(gantt!=null && gantt.getId()!=null){
			Gantt fatherGant =	(Gantt) totalDao.get(Gantt.class, gantt.getId());
			gantt.setId(null);
			gantt.setFatherId(fatherGant.getId());
			gantt.setBelongLayer(fatherGant.getBelongLayer()+1);
			gantt.setRootId(fatherGant.getRootId());
			setProperty(gantt);
	...
	private void setProperty(Gantt gantt){
