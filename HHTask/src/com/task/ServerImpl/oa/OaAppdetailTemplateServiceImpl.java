...
public class OaAppdetailTemplateServiceImpl implements
	...
	//查询分页，筛选
		@SuppressWarnings("unchecked")
		@Override
		public Object[] findExamineList(OaAppDetailTemplate template, String startDate,
				String endDate,Integer cpage,Integer pageSize) {
			//Object[] obj = new Object[2];
			String [] between = new String [2];
			if(null==template){
			...
			String hql = totalDao.criteriaQueries(template, "detailAppDate", between, "1=1 ");
			hql+=" and epId in (select c.id from ExecutionNode e join e.circuitRun c  " + 
					...
					+ "and e.auditUserId="+Util.getLoginUser().getId()+" ) order by detailAppDate desc";
			List<OaAppDetailTemplate> list = totalDao.findAllByPage(hql, cpage, pageSize);
			Integer count = totalDao.getCount(hql);
		...
		public String auditAllShenPi(int[] ids, String tag) {
			...
			if (ids != null && ids.length > 0) {
				...
				for (Integer id : ids) {
					template = (OaAppDetailTemplate) totalDao.getObjectById(OaAppDetailTemplate.class, id);
					if (template != null) {
						if ("ok".equals(tag)) {// 同意
							mess = circuitRunServer.updateExeNodeByCirId(template
						...
						CircuitRun circuitRun = (CircuitRun) totalDao
								.getObjectById(CircuitRun.class, template.getEpId());
						if ("同意".equals(circuitRun.getAllStatus())) {
							template.setEpStatus("同意");
						} else {
							template.setEpStatus("打回");
						}
			...
			if (exclecolums >= 2) {
				...
				for (i = 1; i < exclecolums; i++) {
					...
					if(cells.length>9 && cells[9].getContents()!=null) {
						template.setLendNeckStatus((!"领".equals(cells[9].getContents().trim()) && !
								"借".equals(cells[9].getContents().trim()))?"领":cells[9].getContents().trim());
					}else{
						template.setLendNeckStatus("领");
					}
					list.add(template);
				}
				is.close();// 关闭流
