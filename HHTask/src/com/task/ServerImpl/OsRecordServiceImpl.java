...
public class OsRecordServiceImpl implements OsRecordService {
	...
	@Override
	public String getMaxJcpc(String markId,String selfCard) {
		if(markId!=null && markId.length()>0
				&& selfCard!=null && selfCard.length()>0){
				String max_jcpc =(String) totalDao.getObjectByCondition("select max(jcpc) from OsRecord where markId =? " +
					" and selfCard =? and jcpc like '"+selfCard+"_%' ", markId,selfCard);
				if(max_jcpc!=null && max_jcpc.length()>0){
					String[] strs =	max_jcpc.split("_");
					Integer num =	10000+Integer.parseInt(strs[1])+1;
					max_jcpc=strs[0]+num.toString().substring(1);
				}else{
					max_jcpc = selfCard+"_0001";
				}
				return max_jcpc;
			}
		return "请刷新后重试!~";
