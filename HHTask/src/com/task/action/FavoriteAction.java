...
public class FavoriteAction extends ActionSupport {
	...
	public void showajax() {
		...
		listFavorites = favoriteServer.findAllFavorite(user.getId());
		MKUtil.writeJSON(listFavorites);
	}
	
	public void update(){
		String  jie= favoriteServer.updateDmlAppFileUrl(id);
		if(jie.equals("true")){
			MKUtil.writeJSON("主菜单页面更改成功");
		}else {
			MKUtil.writeJSON("主菜单页面更改失败");
		}
	}
