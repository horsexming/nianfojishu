...
import com.task.entity.menjin.SerialPtSend;
...
public class AccessEquipmentAction {
	...
	private SerialPtReceive ptReceive;
	private SerialPtSend ptSend;
	private String commName;//串口号
	private Integer baudrate;//波特率
	private String sendInfor;//发送数据
	private String types;//send / receive
	private String leitype;//ASCII /Hex
	private static String sendtype = "ASCII";//发送方式
	private static String receiveType = "ASCII";//接收方式
	
	
	public String toDijian(){
		return "dijianLunxun";
	}
	
	public void huoquCHuankou(){
		mCommList = accessEquipmentServer.findCHuankou();
		MKUtil.writeJSON(mCommList);
	}
	public void chengeType(){
		if("send".equals(types)){
			if("Hex".equals(leitype)){
				sendtype = leitype;
			}else {
				sendtype = "ASCII";
			}
		}else if ("receive".equals(types)) {
			if("Hex".equals(leitype)){
				receiveType = leitype;
			}else {
				receiveType = "ASCII";
...
//								ShowUtils.errorMessage("串口对象为空，监听失败！");
								...
								// 保存串口接收的数据
								accessEquipmentServer.addSerialSend(data,mSerialport);
							}
						} catch (Exception e) {
...
//							System.exit(0);
				...
				MKUtil.writeJSON(true,commName+"串口已打开",null);
			}else {
				MKUtil.writeJSON(false,commName+"串口不存在",null);
			}
		} catch (PortInUseException e) {
			System.out.println(commName+"串口已被占用！");
			MKUtil.writeJSON(false,commName+"串口已被占用！",null);
		}
	}
	...
	public void closeSp(){
		if(mSerialport!=null){
			...
			mSerialport=null;
			System.out.println(commName+"串口已关闭");
			MKUtil.writeJSON(true,commName+"串口已关闭",null);
		}else {
			System.out.println("串口关闭失败");
			MKUtil.writeJSON(false,commName+"串口关闭失败",null);
		}
	}
	
	public void sendMess(){
		// 以字符串的形式发送数据
		if (sendtype.equals("ASCII")) {
			SerialPortManager.sendToPort(mSerialport, sendInfor.getBytes());
		}else 
		// 以十六进制的形式发送数据
		if (sendtype.equals("Hex")) {
			SerialPortManager.sendToPort(mSerialport, ByteUtils.hexStr2Byte(sendInfor));
		}else {
			SerialPortManager.sendToPort(mSerialport, sendInfor.getBytes());
		}
//		MKUtil.writeJSON(false,commName+"串口关闭失败",null);
	...
	public void dijian() {
		...
		errorMessage = accessEquipmentServer.updateShow(receiveType,mSerialport);
...
//		accessEquipmentServer.closeShuiFa();
	...
	public List<String> getmCommList() {
		return mCommList;
	}

	public void setmCommList(List<String> mCommList) {
	...
	public static void setmSerialport(SerialPort mSerialport) {
		AccessEquipmentAction.mSerialport = mSerialport;
	}

	public static void setSendtype(String sendtype) {
		AccessEquipmentAction.sendtype = sendtype;
	}
	...
	public String getSendInfor() {
		return sendInfor;
	}

	public void setSendInfor(String sendInfor) {
	...
	public static String getReceiveType() {
		return receiveType;
	}

	public static void setReceiveType(String receiveType) {
		AccessEquipmentAction.receiveType = receiveType;
	}
