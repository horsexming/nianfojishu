 ...
 *
 */
public class ProcardTemplateChangeLog  implements java.io.Serializable{
	private static final long serialVersionUID =1L;
	private Integer id;
	private Integer ptbbId;//设变单对应明细Id
	...
	private Set<ProcardTemplateChangeLogDetail> changeLogDetailSet;//字表
	...
	public Integer getPtbbId() {
		return ptbbId;
	}
	public void setPtbbId(Integer ptbbId) {
		this.ptbbId = ptbbId;
	}
	...
	public Set<ProcardTemplateChangeLogDetail> getChangeLogDetailSet() {
		return changeLogDetailSet;
	}
	public void setChangeLogDetailSet(
			Set<ProcardTemplateChangeLogDetail> changeLogDetailSet) {
	 ...
	 * @return
	 */
	public static String addchangeLog(TotalDao totalDao,ProcardTemplate pt,Object entity,String entityType,String opType,Users user,String nowtime){
		//添加修改日志
		ProcardTemplateChangeLog changeLog = new ProcardTemplateChangeLog();
		Integer banci = pt.getBanci();
		if(banci ==null){
			banci=0;
		}
		ProcardTemplateBanBen ptbb =ProcardTemplateGyServerImpl.getPtbbBypt(totalDao,pt);
		if(ptbb!=null){
			changeLog.setPtbbId(ptbb.getId());
			changeLog.setSbNumber(ptbb.getProcardTemplateBanBenApply().getSbNumber());
		}
		...
		if(entityType.equals("工序")){
			ProcessTemplate process = (ProcessTemplate) entity;
			...
			ProcardTemplate son = (ProcardTemplate) entity;
			...
			ProcessTemplateFile file = (ProcessTemplateFile) entity;
...
//	 * @return
//	 */
//	public static String addchangeLog(TotalDao totalDao,ProcardTemplate pt,ProcessTemplateFile file,String entityType,String opType,Users user,String nowtime){
//		//添加修改日志
//		ProcardTemplateBanBen ptbb =ProcardTemplateGyServerImpl.getPtbbBypt(totalDao,pt);
//		ProcardTemplateChangeLog changeLog = new ProcardTemplateChangeLog();
//		Integer banci = pt.getBanci();
...
//	}
	...
	public static String addchangeLog(TotalDao totalDao,
			ProcardTemplate oldProcardTem, ProcardTemplate procardTemplate2,
			Users user, String nowtime) {
		// TODO Auto-generated method stub
		//添加修改日志
		ProcardTemplateBanBen ptbb =ProcardTemplateGyServerImpl.getPtbbBypt(totalDao,oldProcardTem);
		String sbSql = null;
		if(ptbb==null){
			sbSql = " and ptbbId is null" ;
		}else{
			sbSql = " and ptbbId ="+ptbb.getId() ;
		}
		...
		ProcardTemplateChangeLog changeLog = (ProcardTemplateChangeLog) totalDao.getObjectByCondition("from ProcardTemplateChangeLog where sbMarkId=? and entityType='本身' " +
				"and optype='修改' "+sbSql+banciSql , oldProcardTem.getMarkId());
		if(changeLog==null){
			changeLog= new ProcardTemplateChangeLog();
			if(ptbb!=null){
				changeLog.setPtbbId(ptbb.getId());
				changeLog.setSbNumber(ptbb.getProcardTemplateBanBenApply().getSbNumber());
			}
			...
			changeLog.setAddUsercode(user.getCode());//
			changeLog.setDataStatus("正常");//正常,删除
			Set<ProcardTemplateChangeLogDetail> detailSet = ProcardTemplate.diffrentTwoPt(oldProcardTem, procardTemplate2, changeLog);
			changeLog.setChangeLogDetailSet(detailSet);
			totalDao.save2(changeLog);
		}else{
			Set<ProcardTemplateChangeLogDetail> detailSet = ProcardTemplate.diffrentTwoPt(oldProcardTem, procardTemplate2, changeLog);
			Set<ProcardTemplateChangeLogDetail> olddetailSet =  changeLog.getChangeLogDetailSet();
	...
	public static String addchangeLog(TotalDao totalDao,
			ProcessTemplate oldProcessTem, ProcessTemplate processTempalte,
			Users user, String nowtime) {
		// TODO Auto-generated method stub
		ProcardTemplate pt = oldProcessTem.getProcardTemplate();
		//添加修改日志
		ProcardTemplateBanBen ptbb =ProcardTemplateGyServerImpl.getPtbbBypt(totalDao,pt);
		String sbSql = null;
		if(ptbb==null){
		...
		ProcardTemplateChangeLog changeLog = (ProcardTemplateChangeLog) totalDao.getObjectByCondition("from ProcardTemplateChangeLog where sbMarkId=? and entityType='工序' " +
				"and optype='修改' and entityId in (select id from ProcessTemplate where processNO =? and procardTemplate.markId=? " +banciSql2 + ")"
				+sbSql+banciSql , pt.getMarkId(),oldProcessTem.getProcessNO(),pt.getMarkId());
		if(changeLog==null){
			changeLog= new ProcardTemplateChangeLog();
			if(ptbb!=null){
				changeLog.setPtbbId(ptbb.getId());
			...
			changeLog.setAddUsercode(user.getCode());//
			changeLog.setDataStatus("正常");//正常,删除
			Set<ProcardTemplateChangeLogDetail> detailSet = ProcessTemplate.diffrentTwoPt(oldProcessTem, processTempalte, changeLog);
			changeLog.setChangeLogDetailSet(detailSet);
			totalDao.save2(changeLog);
		}else{
			Set<ProcardTemplateChangeLogDetail> detailSet = ProcessTemplate.diffrentTwoPt(oldProcessTem, processTempalte, changeLog);
