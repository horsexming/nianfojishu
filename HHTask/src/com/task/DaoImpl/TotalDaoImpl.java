...
public class TotalDaoImpl extends HibernateDaoSupport implements TotalDao {
	...
	private void addLogging(Object obj, String status) {
		...
		try {
			logging.setMore(JSON.toJSONString(map));// 转换Map为json格式   --利用C++反射回来后使用JSON报错null
		} catch (Exception e) {
			logging.setMore(new Gson().toJson(map));
		}
...
//													.lastIndexOf("!"));
//								}
//								morevalue += "" + propertyName + " = '"
//										+ propertyStrone + "'";
//							}
//						}
//						if(morevalue.length()>0){
//							hql_str += " and ("+morevalue+" )";
//						}
//						hql2.append(hql_str);
					}
				}
	...
	public List findAllByPageShowCloumn(String hql, int pageNo, int pageSize,
			String showCloumn, Object... obj) {
		if(showCloumn!=null && showCloumn.length()>0){
			String[] hqlArray =	hql.split("from");
			hql ="from "+hqlArray[1];
			String[] cloumArrays = showCloumn.split(",");
			String cloums = "";
			for (String cloum : cloumArrays) {
				cloums+=","+cloum;
			}
			if(cloums.length()>=1){
				cloums = cloums.substring(1);
			}
			hql = "select "+cloums+" "+hql;
		}
			return findAllByPage(hql, pageNo, pageSize, obj);
	}
//	private String getClassName(String hql){
//		if(hql!=null && hql.length()>0){
//			String str =hql.replaceAll(" ", "");
//			String[] strArry =	str.split("from");
//			String[] strArry1 =	strArry[1].split("where");
//			return strArry1[0];
//		}
//		return "";
//		
