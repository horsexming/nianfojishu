						...
						<form id="waiForm" style="margin: 0px; padding: 0px;">
							...
							<table class="table" style="width: 100%;">
								...
								<tr>
									...
									<td>
										<input type="text" value="${procardTemplate.sunhao}"  id="sunhao"
											name="procardTemplate.sunhao" onchange="numyanzheng(this)">
										%
										<span id="zyl_msg" style="color:red;"></span>
									</td>
								</tr>
								<tr>
									<th align="right">
							...
							.each(
									function() {
										...
										var html='<tr align="center"  onmouseover=chageBgcolor(this)  onmouseout=outBgcolor(this,"#FFFFFF") ><td>'
																...
																+ ');">修改</a>/<a href="javascript:;" onclick= "showWgProcard('+$(this).attr('id')+');">关联零件</a>/<a href="javascript:;" onclick="deleteProcess('
																+ $(this).attr(
																		'id')
																+ ');">删除</a></td></tr>';
										$("#ProcessTab")
...
function wgjgx(){
	if(window.confirm("是否关联外购件工序?")){
		...
		$
			.ajax( {
				type : "POST",
				url : "procardTemplateGyAction_processAndWgProcard.action",
				data : {
					id:${param.id}
				},
				dataType : "json",
...
function download(){
	var id=$("#cardId").val();
	window.location.href = "procardTemplateGyAction_findPicbyMarkId.action?id="
			+ id ;
}
...
function gxtbsc(obj){
	if (window.confirm('确定要同步工序到生产吗?')) {
		...
		$.ajax( {
			type : "POST",
			url : "ProcardTemplateAction!gxtbsc.action",
			dataType : "json",
			data : {
				id : $("#cardId").val()
			},
