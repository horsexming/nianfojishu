				...
				<form action="WarehouseApplicationAction_add.action"
					...
					<table class="table">
						<tr style="width: 100%">
							...
							<td align="center">
								<input type="text" name="warehouseApplication.selfCard" value="${oadetail.detailAppName}"
								...
								<input type="hidden" name="warehouseApplication.cuiqutype" value="存"
									id="type" readonly="readonly"/>
								<input type="hidden" name="warehouseApplication.classNameId" value="${oadetail.id}"
									id="classNameId" readonly="readonly"/>
								<input type="hidden" name="warehouseApplication.className" value="${oadetail.classNames}"
							...
							<td align="center">
								<input type="text" name="warehouseApplication.totalNumber"
									id="totalNumber" value="${oadetail.detailCount}" readonly="readonly"/>
							</td>
						</tr>
						<tr>
							<th >单个库位存放量</th>
...
function shengchengtr(){
	...
	if(dangeNum!="" && dangeNum>0 && totalNumber != "" && totalNumber>0){
		...
		for(var i=0;i<index;i++){
			...
			'<SELECT id="kuwei" style="width: 153px;" name="warehouseCertificateList['+i+'].warehouseId"  ><option></option><s:iterator value="wnList" id="wn" status="pageStatus">' +
