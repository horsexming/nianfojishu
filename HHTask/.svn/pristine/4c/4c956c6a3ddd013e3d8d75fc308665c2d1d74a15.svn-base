				...
				<div class="row">
					<div class="col-xs-12">
						<div class="form-group">
							...
							<br>
							<s:iterator value='{"生产","项目","KVP","其它"}' id="ott">
								<label class="radio-inline">
								<s:if test="overtime.overtimeType==#ott">
									<input type="radio" name="overtime.overtimeType" 
										value="${ott}" checked="checked" onchange="switchCause(this);" />${ott}
								</s:if>
								<s:else>
									<input type="radio" name="overtime.overtimeType" 
										value="${ott}" onchange="switchCause(this);" />${ott}
								</s:else>
								</label>
							</s:iterator>
						</div>
						...
						<div class="form-group">
							...
							<div class="content row col-xs-12">
								<select name="overtime.overtimeNeirong" id="overtimeNeirong">
									<option>${overtime.overtimeNeirong}</option>
								</select>
							</div>
						</div>
					</div>
...
$jq(function() {
	$("#showDialogs").hide();
	var neirong = $('input[name="overtime.overtimeType"]:checked').val();
	if(neirong=="生产"){
		...
		var overtimeNeirong = $("#overtimeNeirong").val();
		$(".content").empty();
		$(".content").append('<textarea class="form-control" rows="3" id="overtimeNeirong"'+
				'name="overtime.overtimeNeirong">'+overtimeNeirong+'</textarea><br>');
	}
