			...
			<div id="closeDiv"
				...
				<div id="operatingDiv"
					...
					<iframe id="showProcess" src="" marginwidth="0" marginheight="0"
						hspace="0" vspace="0" frameborder="0" scrolling="yes" height="1000px"
						...
						<tbody>
							<tr>
								<td align="left" colspan="21">
									<h2>
										我的欠款金额:
										<font style="color: red">${backMoney}元</font>
										<input type="button" value="冲抵借款" onclick="tocz()">
									</h2>
								</td>
							</tr>
							<tr>
								...
								<td colspan="1">
									<input type="radio" id="zhifuoryufu1"
										...
										onclick="showzhifu_tr()" checked="checked" />
							...
							<tr>
								...
								<td colspan="2">
									...
									否
									<input type="radio" id="jituan2" value="是"
										name="fundApply.jituan" />
									是
								</td>
							</tr>
							<tr class="zhifu_tr">
							...
							<tr>
								...
								<td colspan="7">
									<input type="radio" id="voucherbasis1"
										name="fundApply.voucherbasis" value="合同" onclick="showHt()" />
							...
							<tr class="zhifu_tr">
								...
								<td>
									<select name="fundApply.isTax" class="zhifu_" class="zhifu_"
										id="isTax" disabled="disabled" onchange="istax()">
<%--										<option value="单据">--%>
<%--											单据--%>
<%--										</option>--%>
										<option value="发票">
							...
							<tr>
								...
								<td colspan="7">
									<input type="radio" name="fundApply.jingbanren"
										onclick="javascript:$('#showname').show();" value="是">
									是
									<input type="radio" name="fundApply.jingbanren"
										checked="checked" onclick="javascript:$('#showname').hide();"
										value="否">
									否
									<label id="showname" style="display: none; color: red">
										经办人姓名:${Users.name}
									</label>
								</td>
							</tr>
							...
							<tr align="center">
								...
								<td>
									<textarea cols="40" rows="2" id="invoiceNumd0" class="zhifu_fp"
										onclick="selectfapiao('0')" disabled="disabled"
										name="fundApply.fadList[0].invoiceNo" readonly="readonly"
										placeholder="点击选择发票号"></textarea>
									<input type="button" value="清空" onclick="delfapiao('all','0')" />
								</td>
							</tr>
							<tr id="uploadtr">
								<th align="left">
								...
								<td colspan="2">
									...
									<input id="allkyMoney" value="0">
								</td>
							</tr><tr class="zhifu_tr" id="in1">
								<th>
									发票号码
								</th>
								<td colspan="7">
...
function saveHKInfor(obj, few) {
	...
	var html = '<tr>'
			...
			+ lineCount
			+ '].fk_SubBudgetRateId" id="fk_SubBudgetRateId'
			+ lineCount
			+ '">'
			...
			+ '].pay_use" id="pay_use'
			+ lineCount
			+ '"></textarea></td><td ><textarea cols="40" rows="2" id="invoiceNumd'
			+ lineCount
			+ '" class="zhifu_fp" onclick="selectfapiao(&apos;'
			+ lineCount
			+ '&apos;)" '
			+ ' name="fundApply.fadList['
			+ lineCount
			+ '].invoiceNo"  placeholder="点击选择发票号"></textarea>'
			+ '<input type="button" value="清空" onclick="delfapiao(&apos;all&apos;,&apos;'
			+ lineCount + '&apos;)"/></td></tr>';
	lineCount++;
...
function hidzhifu_tr(){
	...
	$(".zhifu_fp").attr("disabled", "disabled");
	$("#voucherbasis2").hide();
...
function selectfapiao(num) {//OCR
	var fapiaohao = $("#invoiceNum").val();
	var fapiaohao2 = $("#invoiceNumd"+num).val();
	$("#title").val("您正在选择发票号码");
	var url = "FundApplyAction_findInvoceCheckXuan.action?pagestatus=allCode&wwNumber="+num+"&fapiaohao="+fapiaohao+"&bwtCompany="+fapiaohao2;
...
//删除输入的发票号码
function delfapiao(hao,num){
	if(hao=='all'){
		delfapiaoone(num);
		$("#invoiceNumd"+num).val('');
		$("#h"+num).val('');
	}else{
		var delinvoice = $("#invoiceNumd"+num).val();
		var de = ","+delinvoice+",";
		var fapiaohao = $("#invoiceNum").val();
...
function delfapiaoone(num){
		var invoiceNumdval=$("#invoiceNumd"+num).val().split(",");
		for(var i=0;i<invoiceNumdval.length-1;i++){
			var delinvoice = invoiceNumdval[i];
			var de = ","+delinvoice+",";
			var fapiaohao = $("#invoiceNum").val();
			if(fapiaohao.indexOf(de)>=0){
				fapiaohao = fapiaohao.replaceAll(delinvoice+",","");
