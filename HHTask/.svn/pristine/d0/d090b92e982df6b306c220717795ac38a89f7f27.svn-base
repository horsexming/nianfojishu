...
<%--				</s:if>--%>
				<form action="ManualOrderPlanAction_findManualPlanTotal.action"
					...
					<input type="hidden" value="${mod1.addUsersCode}" name="mod1.addUsersCode">
...
<%--						class="input" />--%>
					...
					<table class="table">
						...
						<s:iterator value="planTotalList" id="ptotal" status="pagestatus">
							...
							<td>${ptotal.pickingStatus}</td>
							<td>
								<s:if test="#ptotal.pickingStatus==null || #ptotal.pickingStatus!='已领完'">
									<input type="button" value="领料" onclick="location.href='ManualOrderPlanAction_manualPlanTotalPrint.action?id=${ptotal.id}&status=${status }'"/>
								</s:if>
								<s:else>
