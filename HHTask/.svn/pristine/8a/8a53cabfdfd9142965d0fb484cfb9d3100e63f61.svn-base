			...
			<div align="center">
				<div id="xitong"
					...
					<div style="float: left; width: 45%; padding-top: 5px;"
						align="right">
						<a href="dimissionLogAction_showList.action"
				...
				<div align="center">
					...
					<form action="dimissionLogAction_showList.action?tag=${tag}" method="post">
					...
					<table  class="table">
						...
						<s:iterator value="dimissionLogList" id="dimisl"
							...
							<td colspan="2">
								<a
									href="dimissionLogAction_toselect.action?dimissionLog.id=${dimisl.id}&tag=${tag}&id=${dimisl.epId}">预览</a>
								<s:if test="#dimisl.epId!=null">
									<a href="CircuitRunAction_findAduitPage.action?id=${dimisl.epId}">/审批动态</a>
								</s:if>
								<s:if test="#dimisl.hr_status!='已填写'&&#dimisl.lzsq_status!='打回'">
								<a
									href="dimissionLogAction_toupdate.action?dimissionLog.id=${dimisl.id}&tag=all">/人事项</a>
								</s:if>
								<input type="hidden" value="toupdate_"/>
								<input type="hidden" value="delete_cpage=${cpage}"/>
								
								<s:if
									test="tag=='admin'">
									<a
										href="dimissionLogAction_toupdate.action?dimissionLog.id=${dimisl.id}&tag=${tag}">/修改</a>
									<a onclick="return window.confirm('您将删除数据，是否继续?')"
										href="dimissionLogAction_delete.action?dimissionLog.id=${dimisl.id}&cpage=${cpage}&tag=${tag}">/删除</a>
								</s:if>
								<%--
								<s:if test="#dimisl.zhengyi_Status!=null&&#dimisl.zhengyi_Status=='待填写'">
									<a href="dimission_ZhengYiAction_toadd.action?dimissionLog.id=${dimisl.id}">/<font color="#ff0000">添加离职争议单</font></a>
								</s:if>
								
								<s:if
									test="#dimisl.hand_status!=null&&#dimisl.hand_status=='未填写'">
									<a
										href="dimission_HandoverAction_toadd.action?dimissionLog.id=${dimisl.id}">/<font
										color="#ff0000">添加离职交接单</font>
									</a>
								</s:if>--%>
							</td>
