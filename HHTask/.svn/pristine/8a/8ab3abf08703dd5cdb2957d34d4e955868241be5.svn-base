 ...
 * Dao层实现类
 */
public class TotalDaoImpl_2 extends HibernateDaoSupport implements TotalDao_2 {
	...
	private void addLogging(Object obj, String status) {
		...
		if (loginUser != null) {
			...
			Admin admin = (Admin) ActionContext.getContext().getSession().get(
					"admin");// 获得登录用户信息
			if (admin != null) {
			...
			try {
				out: for (Field field : fields) {
					try {
						...
						if (propertyObj != null) {
							if (propertyObj.toString().trim().equals("[]")
									|| "".equals(propertyObj.toString())) {
								continue;
							}
	...
	public String criteriaQueries(Object obj, String betweenName,
		...
		if (obj != null) {
			...
			try {
				...
				if (between != null && null != between[0] && null != between[1]
						&& between[0].length() > 0 && between[1].length() > 0) {
					hql2 += " and " + betweenName + " between '" + between[0]
							+ "' and '" + between[1] + "'";
				}
