			...
			<div align="center">
				...
				<div align="center">
					<s:if test="pageStatus!=null">
						请选择人脸识别摄像头
						<select name="pageStatus" onchange="changeCamera()" id="pageStatus">
							<s:iterator value="cameraList" id="cl" status="ps">
								<s:if test="pageStatus!=null && pageStatus.equals('face_'+#cl.id + '.jpg')">
									<option value="face_${cl.id}.jpg" selected="selected">${cl.name }</option>
								</s:if>
								<s:else>
