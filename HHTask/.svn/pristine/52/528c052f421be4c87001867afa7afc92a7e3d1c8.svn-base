...
<!--						</td>-->
				...
				<table class="table">
					...
					<s:iterator value="noPriceprocessList" id="list"
						...
						<td align="center">
						<s:if test="#list.stutas=='同意'">
							<a onclick="checkZhuser(${list.id});">录入价格</a>
						</s:if>
						</td>
					</s:iterator>
					<tr>
		...
		function checkZhuser(id) {
			window.location.href = "NoPriceprocessAction_lvruPrice.action?noPriceprocess.id="
				+ id + "&cpage=${cpage}";
		}
	function detail(id) {
		window.location.href = "NoPriceprocessAction_findZhuserForjiage.action?id="
			+ id 
		}
</script>
	</body>
