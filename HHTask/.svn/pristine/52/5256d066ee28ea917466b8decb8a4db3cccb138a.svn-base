...
<html>
	<head>
		<%@include file="/util/sonHead.jsp"%></head>
	...
	<body onload="addMessage">
		...
		<div id="gongneng"
			...
			<div align="center">
				<form action="ProcardTemplateAction!findPrintProcessList.action"
					method="post">
					<table width="95%">
						<tr>
							<td>
								零件号：
								<select name="procardTemplate.markId" id="markId"
									style="width: 140px;"
									onMouseOver="createDept('markId','RunningWaterCardAction!selectItem.action?tag=markId')">
									<option value=""></option>
									<option value="${procardTemplate.markId}" selected="selected">
				...
				<form action="ProcardTemplateAction!selestedPrintList.action"
					...
					<table width="900px" class="table">
						<tr>
							<td align="right" colspan="6">
								<font color="red">共选择 <label id="peopleLabel">
										...
										style="width: 20px;" readonly="readonly"> 道打印工序</font>
						...
						<s:iterator value="list" status="se" id="process">
							...
							<td>
							<s:if test="%{'总成'==process.procardTemplate.procardStyle}">
								${process.procardTemplate.markId}
							</s:if>
							<s:else>
							${process.procardTemplate.procardTemplate.markId}
							</s:else>
