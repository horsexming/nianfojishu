...
<html>
	...
	<body>
		<form id="xform"
			action="GzstoreAction_addProcess4.action?id1=${gzstore.id}	"
			...
			<table class="table">
				...
				<tr>
					...
					<td>
						<input type="text" id="matetag" name="processGzstore.processName"
							value=""  onchange="isvalue()"/><font color="red" id="zifont"></font>
					</td>
					<th align="right">
						生产类型:
					</th>
				...
				<tr>
					...
					<td rowspan="7">
						<textarea rows="8" cols="30" name="processGzstore.more"> </textarea>
					</td>
...
function addFuliaoLine() {
	...
	var html = "<tr id='fuliaoTr"
			...
			+ "\","
			+ maxFuLiaoIndex
			+ ")' onBlur='hidediv(\"showAll0\")' name='processGzstore.fuliaoList["
			+ maxFuLiaoIndex
			+ "].name' /> </td>"
...
function validate(){
	var matetag = document.getElementById("matetag");
	if(matetag!=null && matetag.value==""){
		$("#zifont").html("请填写工序名称");
		return false;
	}
