						...
						<s:if test="pageStatus!='noUpload'&&pageStatus!='view'">
							<tr>
								...
								<td align="left">
									...
									下发到工序：
									<input type="radio" checked="checked" name="ytRadio" id="yt"
										value="yes">
									是
									<input type="radio" name="ytRadio" id="zkt" value='no' 
									${quotedPrice.procardStyle=="外购"?'checked=checked':""}>
									否
								</td>
						...
						<s:iterator value="list" id="spFile">
							<s:if test="#spFile.type=='视频文件'">
								<tr>
									<td align="left">
										<video
											src="<%=path%>/upload/file/bjTz/${spFile.month}/<s:property value="#spFile.fileName"/>"
											controls="controls">
										您的浏览器不支持 video 标签。
										</video>
									</td>
									...
									<a
											href="QuotedPrice_DownloadProcessTz.action?id=${spFile.id}">下载</a>
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${spFile.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deleteProcessTz.action?quotedPriceFile.id=${spFile.fileName.id}&pageStatus=card&id=${param.id}" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
										</s:if>
								</tr>
							</s:if>
						...
						<s:iterator value="list" id="file3D">
							<s:if test="#file3D.type=='3D文件'">
								<tr>
									<td align="left">
										<a
											href="<%=path%>/upload/file/bjTz/${file3D.month}/<s:property value="#file3D.fileName"/>">
											<img
												src="<%=path%>/upload/file/bjTz/${file3D.month}/<s:property value="#file3D.fileName"/>"
									...
									<s:if test="tag==null||tag!='noxz'">
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${file3D.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${file3D.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deletebjTz.action?quotedPriceFile.id=${file3D.id}&&id=${param.id}&&pageStatus=card" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
										</s:if>
								</tr>
							</s:if>
						...
						<s:iterator value="list" id="tzFile">
							<s:if test="#tzFile.type=='工艺规范'">
								<tr>
									<td align="left">
										<a target="_showPri"
<%--											href="<%=path%>/upload/file/bjTz/${tzFile.month}/<s:property value="#tzFile.fileName"/>">--%>
											href="<%=path%>/FileViewAction.action?FilePath=/upload/file/bjTz/${tzFile.month}/<s:property value="#tzFile.fileName"/>">
											
											<img
												src="<%=path%>/upload/file/bjTz/${tzFile.month}/<s:property value="#tzFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<s:if test="tag==null||tag!='noxz'">
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${tzFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${tzFile.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deletebjTz.action?quotedPriceFile.id=${tzFile.id}&pageStatus=card&id=${param.id}" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
										</s:if>
								</tr>
							</s:if>
						...
						<s:iterator value="list" id="sopFile">
							<s:if test="#sopFile.type=='SOP文件'">
								<tr>
									<td align="left">
										<a target="_showPri"
											href="<%=path%>/upload/file/bjTz/${sopFile.month}/<s:property value="#sopFile.fileName"/>">
											<img
												src="<%=path%>/upload/file/bjTz/${sopFile.month}/<s:property value="#sopFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<s:if test="tag==null||tag!='noxz'">
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${sopFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${tzFile.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deleteProcessTz.action?quotedPriceFile.id=${tzFile.id}&pageStatus=card&id=${param.id}" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
										</s:if>
								</tr>
							</s:if>
						...
						<s:iterator value="list" id="sapFile">
							<s:if test="#sapFile.type=='SIP文件'">
								<tr>
									<td align="left">
										<a target="_showPri"
											href="<%=path%>/upload/file/bjTz/${sapFile.month}/<s:property value="#sapFile.fileName"/>">
											<img
												src="<%=path%>/upload/file/bjTz/${sapFile.month}/<s:property value="#sapFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<s:if test="tag==null||tag!='noxz'">
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${sapFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${sapFile.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deleteProcessTz.action?quotedPriceFile.id=${sapFile.id}&pageStatus=card&id=${param.id}" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
										</s:if>
								</tr>
							</s:if>
						...
						<s:iterator value="list" id="modelFile">
							<s:if test="#modelFile.type=='3D模型'">
								<tr>
									<td align="left">
										<a
											href="<%=path%>/upload/file/bjTz/${modelFile.month}/<s:property value="#modelFile.fileName"/>">
											<img
												src="<%=path%>/upload/file/bjTz/${modelFile.month}/<s:property value="#modelFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<td>
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${modelFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
						...
						<s:iterator value="list" id="cxFile">
							<s:if test="#cxFile.type=='成型图'">
								<tr>
									<td align="left">
										<a
<%--											href="<%=path%>/upload/file/bjTz/${cxFile.month}/<s:property value="#cxFile.fileName"/>">--%>
											href="FileViewAction.action?FilePath=/upload/file/bjTz/${cxFile.month}/<s:property value="#cxFile.fileName"/>">
											<img
												src="<%=path%>/upload/file/bjTz/${cxFile.month}/<s:property value="#cxFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<td>
										<s:if test="tag==null||tag!='noxz'">
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${cxFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
						...
						<s:iterator value="list" id="qtFile">
							<s:if test="#qtFile.type=='其它文件'">
								<tr>
									<td align="left">
										<a
											href="<%=path%>/upload/file/bjTz/${qtFile.month}/<s:property value="#qtFile.fileName"/>">
<%--											href="<%=path%>/FileViewAction.action?FilePath=/upload/file/bjTz/${qtFile.month}/<s:property value="#qtFile.fileName"/>">--%>
											
											<img
												src="<%=path%>/upload/file/bjTz/${qtFile.month}/<s:property value="#qtFile.fileName"/>"
												style="width: 80px; height: 80px;" />
										</a>
									</td>
									<td align="left">
									...
									<td>
										<a
											href="QuotedPrice_DownloadProcessTz.action?id=${qtFile.id}">下载</a>&nbsp;&nbsp;
										<s:if test="pageStatus!='view'">
											<input type="button"
												onclick="deletetz(${qtFile.id},${param.id})" value="删除" />
											<%--				   <a href="ProcardTemplateAction!deletebjTz.action?quotedPriceFile.id=${modelFile.id}&pageStatus=card&id=${param.id}" onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td>--%>
										</s:if>
								</tr>
							</s:if>
						</s:iterator>
...
function deletetz(fileId, id) {
	if (window.confirm("确认要删除该条记录?")) {
		window.location.href = "QuotedPrice_deleteProcessTz.action?quotedPriceFile.id="
				+ fileId + "&pageStatus=card&id=" + id
	}
...
$(function() {
    $('#accessory').uploadify({
        ...
        'uploader' : 'QuotedPrice_updateQuotedPriceTz.action;jsessionid=<%=session.getId()%>',//请求路径  
	    'auto' 	   : false, //是否自动上传 
	    'debug'    : false,
	    ...
	    'onUploadStart' : function(file) {
	   		 ...
	   		 var ytRadioval=$('input:radio[name="ytRadio"]:checked').val();
	   		 var idval=document.getElementById('id').value;
	  		 var obj={'quotedPriceFile.type' :typeval,'ytRadio':ytRadioval,'id':idval};
