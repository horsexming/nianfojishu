...
<html>
				...
				<form
					...
					<table class="table">
						...
						<s:iterator value="list" status="se" id="bar">
							...
							<td>
								<input type="checkbox" id="${bar.saleMoney}"
									name="detailSelect" value="${bar.id}" 
									onclick="chageNum(this,'${bar.saleMoney}')">
	...
	<script type="text/javascript">
    	function chageType(obj,form){
    		if(obj.id=="ok"){
    			form.action="saleBudgetAction!updateExamBargain.action?tag=ok";
    			form.submit();
    		}else if (obj.id=="ng"){
    			form.action="saleBudgetAction!updateExamBargain.action?tag=ng";
    			form.submit();
    		}
    	}
