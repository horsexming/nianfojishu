				...
				<form action="SetupCheckAction_findAllbyName.action" method="post">
					<table class="table" align="center">
						...
						<th>
							...
							<td align="center">
								<input id="bfName1" name="setupCheck.uploadPerson1"
...
<%--							<a href="<%=path%>/upload/file/detail/${list.shortFile}">审核文件</a>/--%>
							<a href="FileViewAction.action?FilePath=/upload/file/detail/${list.shortFile}">审核文件</a>/
							</s:if><s:else><FONT color="red">无文件</FONT>/</s:else>
							<a onclick="toDetail(${list.id},'${cpage}')">处理措施</a>/
							<a onclick="toFile(${list.id},'${cpage}')">上传佐证</a>
