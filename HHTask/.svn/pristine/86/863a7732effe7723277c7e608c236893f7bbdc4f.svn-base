...
<html>
	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
			<h3>当前审核：${role }</h3>
			<form action="business_queryByCondition.action" method="post" onsubmit="return validate()">
				<table>
					<tr>
						<td>业务类型：</td>
						<td><select name="type">
							<option value="选择类型" selected="selected">选择类型</option>
							<s:iterator id="typeStr" value='{"运输","报关","仓储","仓储配送"}'>
								<s:if test='#typeStr == type'>
									<option value="${typeStr }" selected="selected">${typeStr }</option>
								</s:if>
								<s:else><option value="${typeStr }">${typeStr }</option></s:else>
							</s:iterator>
							</select>	
						</td>
						<td>业务内容：</td>
						...
						<td>收款单位：</td>
						<td><input type="text" name="collectionUnit" value="${collectionUnit }"/></td>
					</tr>
					<tr>
						<td>经办人：</td>
						<td><input type="text" name="transactor" value="${transactor }"/></td>
						<td>开始时间：</td>
						<td><input style="width: 155px" class="Wdate"
					...
					<s:iterator value="list" id="pageList" status="pageStatus">
						...
						<td>
							...
							<s:property value="#pageStatus.index+1" />
									...
									<td><input type="button" value="不同意" style="width: 60px; height: 30px;" onclick="noAgree(${pageList.id })"/></td>
						</tr>
						</s:iterator>
							<tr>
		...
		<script type="text/javascript">
			function invoice(id){
				window.location = "business_queryInvoiceByBusinessId.action?id="+id+"&flow=${flow}";
			}
			function detail(id){
				window.location = "business_queryDetailByBusinessId.action?id="+id+"&flow=${flow}";
			}
			function agree(id){
				window.location = "business_auditProcess.action?id="+id+"&flow=${flow}&ifAgree=1";
			}
			function noAgree(id){
				window.location = "business_auditProcess.action?id="+id+"&flow=${flow}&ifAgree=2";
			}
			function validate(){
