	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
				...
				<form action="procardBlAction_findRootProcardBlbyCondition.action"
					method="post">
				...
				<table  class="table">
					...
					<s:iterator value="procardList" id="pagebl" status="pageStatus">
						...
						<td colspan="2">
							...
							<s:if test="#pagebl.status=='设变锁定'">
							<font color="red">设变锁定中</font>
							</s:if>
							<s:else>
							<input type="button" value="备料" style="height: 20px;width: 80px;" onclick="showDetailForbl(${pagebl.id},this)">
							<input type="button" value="领料" style="height: 20px;width: 80px;" onclick="showDetailForll(${pagebl.id},this)">
							</s:else>
						</td>
					</tr>
					</s:iterator>
...
function showDetailForbl(id,obj){
	$(obj).attr("disabled","disabled");
	window.location.href="procardBlAction_findProcardBlDetailbyRootId.action?id="+id;
}
function showDetailForll(id,obj){
	$(obj).attr("disabled","disabled");
	//window.location.href="procardBlAction_findProcardllDetailbyRootId.action?id="+id;
	window.location.href="procardBlAction_findProcardBlByPId.action?pagestatus=lingliao&rootId="+id;
}
