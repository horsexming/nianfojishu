	...
	$.ajax( {
		...
		success : function(data) {
			if (data.success) {
				...
				if (pageStatus != 'zw') {
					var fanghuOutLib = map.fanghuOutLib;
					if(null==fanghuOutLib.lastLingyongTime){
						$("#num").val(fanghuOutLib.limitCount);
					}else{
						$("#num").val(fanghuOutLib.nextLingyongcount);
					}
					
					$("#fhid").val(fanghuOutLib.id);
				}
		...
		<div id="gongneng"
			...
			<div align="center">
				...
				<form action="store_lend.action" method="post"
					...
					<input name="fhid" id="fhid" type="hidden" />
					<table border="1" style="border-collapse: collapse;">
						<tr>
							...
							<td align="right">
								名称：
								<input type="text" id="matetag" name="vos.matetag"
									value="${store.matetag}" readonly="readonly" />
							</td>
						</tr>
						<tr>
							<td align="left">
								数量：
								<input id="num" type="text" name="vos.num" id="num" />
							</td>
							<td style="width: 30px;"></td>
							<td align="right">
								单位：
						...
						<tr>
							<td align="left">
								规格：
								<input type="text" id="format" name="vos.format"
									value="${store.format}" readonly="readonly" />
							</td>
							<td style="width: 30px;"></td>
							<td align="right">
								类别：
