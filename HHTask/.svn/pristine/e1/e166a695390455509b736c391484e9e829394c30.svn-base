...
public class DmltryAppFilesAction {
	...
	public String addin() {
		if (fujian != null) {
			...
			realFileName = upload.UploadFile(fujian, fujianFileName, null, realFilePath, null);
	...
	public String fenyeandtj() {
		...
		if (dmltryAppFiles != null) {
			ActionContext.getContext().getSession().put("dmlAppFileUrlfy", dmltryAppFiles);
		} else {
			// 保持分页
			dmltryAppFiles = (DmltryAppFiles) ActionContext.getContext().getSession().get("dmltryAppFilesfy");
		}
		/// 注意一下bject[] object
