...
<html>
	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center" 
			style="color:red;font-size:30px;;height:40px;" >全部未通过审核查询页面</div>
			<div align="center">
				<table width="100%" border="0" style="border-collapse: collapse;"
					...
					<s:iterator value="list" id="mealtickets" status="pageStatus">
						...
						<td>
								...
								<s:if test='%{"拒绝".equals(#mealtickets.fuck)}'></s:if><s:else>
								<a onclick="return window.confirm('确定通过审核！！！')"
								 href="mealAction!updatee.action?mealticket.id=${mealtickets.id}">/同意/</a>
								<a href="mealAction!redown.action?mealticket.id=${mealtickets.id}">打回</a>
								</s:else>
						</td>
						</tr>
					</s:iterator>
