		...
		<div id="gongneng"
			...
			<div align="center">
				<table class="table" width="100%" border="0" style="border-collapse: collapse;">
					...
					<s:iterator value="equipmentDepreciations" id="r" status="st">
						...
						<s:else>
							...
							<td>${r.process}</td>
							<td>${r.equipment}</td>
							<td>${r.equipmentValue}</td>
							<td>${r.endLifeValue}</td>
							<td>${r.lifeTime}</td>
							<td>${r.workTime}</td>
							<td>${r.workDay}</td>
							<td>${r.tTS}</td>
							<td>${r.equipmentDepreciation}</td>
							<td>
								<a href="ProjectQuotationEquipmentDepreciation_updateInput.action?equipmentDepreciation.id=${r.id}">修改</a>
