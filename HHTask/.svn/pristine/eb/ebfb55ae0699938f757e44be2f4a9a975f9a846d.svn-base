...
function findPro(fatherId, belongLayer, index) {
	...
	$
			.ajax( {
				async : false,
				type : "post",
				url : "gyslgxAction!findProByBel.action",
				data : {
					id : fatherId,
						...
						$
								.each(obj, function(i, procard) {
									...
									//判断激活状态
										...
										if (tjNumber != null) {
											width = (procard.tjNumber
													/ procard.filnalCount * 100)
													.toFixed(0);
											nextWidth = 100 - width;
										} else {
											tjNumber = 0;
										...
										var nextWidthCon = procard.proName
												+ "(<b>" + procard.procardStyle
												+ "</b>)" + flstatus
												+ procard.markId + "(<b>完成:"
												+ tjNumber + "/"
										...
										//递归生成
										var sonHeight = findPro(procard.id,
												belongLayer + 1, i);
										if (sonHeight > 54) {
...
function showProcessForSb(id, jihuoStatus, proStatus) {
	...
	if ('${pageStatus}' == 'history') {
		$("#showProcess").attr(
				"src",
				"gyslgxAction!findProcardGysByRunCard2.action?id=" + id
		...
		if (jihuoStatus != "wei") {
			if (proStatus != "初始" && proStatus != "已发卡") {
				$("#showProcess")
						.attr(
								"src",
								"gyslgxAction!findProcardGysByRunCard2.action?id="
										+ id
										+ "&pageStatus=${pageStatus}");
				chageDiv('block');
			}
	...
	<body style="background-color: #ffffff;">
		...
		<center>
			<div align="center"
				...
				${procardGys.proName}(件号:${procardGys.markId} 批次:${procardGys.selfCard})的生产进度
				(
