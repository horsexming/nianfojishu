...
public class DeptNumberServerImpl implements DeptNumberServer {
	...
	public String updateDept(DeptNumber deptNumber, DeptNumber oldDeptNumber) {
		if (oldDeptNumber != null) {
			...
			oldDeptNumber.setIsVisitor(deptNumber.getIsVisitor());
			if(deptNumber.getIsVisitor()!=null && deptNumber.getIsVisitor().equals("是")) {
				DeptNumber fatherDeptNumber= (DeptNumber) totalDao.getObjectById(DeptNumber.class, oldDeptNumber.getFatherId());
				while (fatherDeptNumber!=null) {
					if(fatherDeptNumber.getIsVisitor()==null || fatherDeptNumber.getIsVisitor().equals("否")) {
						fatherDeptNumber.setIsVisitor("是");
						totalDao.update(fatherDeptNumber);
					}
					if(fatherDeptNumber.getFatherId()!=null) {
						fatherDeptNumber = (DeptNumber) totalDao.getObjectById(DeptNumber.class, fatherDeptNumber.getFatherId());
					}else {
						fatherDeptNumber=null;
					}
