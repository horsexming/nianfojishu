			...
			<div align="center">
				...
				<table  class="table">
					...
					<s:iterator value="list" id="pageList" status="pageStatus">
						...
						<td>
						<input type="button" value="采购"
								style="width: 60px; height: 30px;"
								onclick="caigou(${pageList.id})" />
...
/*		function caigou(id){
			var url=encodeURI(encodeURI("DingdanAction!caigou.action?internalOrderzhaobiao.id="+id));
		     $("#showProcess").attr("src", url);	
		    chageDiv('block');
	}*/
	function caigou(id) {
	window.location = "DingdanAction!caigou.action?internalOrderzhaobiao.id=" + id;
}
	
	/*	
function detail(id) {
	window.location = "DingdanAction!listInternalOrderDetail_hedui.action?internalOrderzhaobiao.id=" + id
...
*/
  ...
  //入库操作
  function ruku(id) {
	window.location = "DingdanAction!ruku.action?internalOrderzhaobiao.id=" + id;
}
