...
.red{
   background-color: #FFFF33;
	...
	<body>
	    <!-- 设置一个隐藏的dateTemp -->
	    <input id="dateTemp" type="hidden" value="${dateTemp}" />
...
var dateTemp = $("#dateTemp").val();
//---------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------
//下一周的图片点击事件
$("#xyz").click(function(){
	var dateTemp1 =1;
	dateTemp = parseInt(dateTemp)+1;
	location.href = "ClearInfoAction_selectOnTheJob.action?dateTemp1=" + dateTemp1+"&dateTemp="+dateTemp;
	...
	//获取其人员Id value值
		...
		$.ajax( {
			...
			success : function(data){//成功时执行的代码
				if (data==1) {
					alert("添加打扫成功!!!");
					window.location.href="ClearInfoAction_selectOnTheJob.action?dateTemp="+dateTemp+"&dateTemp1=1";
				} else {
				    alert("人员已存在!!!");
				}
			}
...
$(".empName").click(function() {
		...
		$.ajax( {
			...
			success : function(data) {//成功时执行的代码
				alert("已取消!!!");
				location.href = "ClearInfoAction_selectOnTheJob.action?dateTemp="+dateTemp+"&dateTemp1=1";				
			}
		});
	})
