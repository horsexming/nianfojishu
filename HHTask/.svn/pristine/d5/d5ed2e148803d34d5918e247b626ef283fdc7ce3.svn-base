...
/***
 * 流水卡片模板(表名:ta_sop_w_ProcardTemplate)
 * 
 * @author 刘培
 * 
 */
public class ProcardTemplate  implements Serializable {
	...
	private Float xiaohaoCount;//单台用量（目前黎阳使用）
	...
	private String fatherMarkId;//上层件号
	...
	private Set<ProcardTemplate> procardTSet;// 下层流水卡片信息(本表的一对多关系)
	private Set<ProcessTemplate> processTemplate;// 对应的工序信息(一对多关系)
	private Set<ZhUser> zhUsers;
	...
	@JSONField(serialize = false)
	public Set<ProcardTemplate> getProcardTSet() {
		return procardTSet;
	}

	public void setProcardTSet(Set<ProcardTemplate> procardTSet) {
	...
	@JSONField(serialize = false)
	public Set<ZhUser> getZhUsers() {
		return zhUsers;
	}

	public void setZhUsers(Set<ZhUser> zhUsers) {
		this.zhUsers = zhUsers;
	}
	...
	public boolean isDanJiaojian() {
		if (this.procardStyle != null && this.procardStyle.equals("总成")
				&& this.danjiaojian != null && this.danjiaojian.equals("单交件")) {
			return true;
		}
		return false;
	}
	 ...
	 * 
	 * @param type
	 *            0不领料时返回true否则返回不领料,不计算
	 * @return
	 */
	public String sumXiaoHaoCount(Integer type) {
		if (this.procardStyle == null
				|| (!isDanJiaojian() && this.procardStyle.equals("总成"))
				|| (!this.procardStyle.equals("自制") && !this.procardStyle
						.equals("组合"))) {
			if (type == 0) {
				return "true";
			}
		...
		if (maxCount != null && maxCount > 0) {
			if (quanzi1 != null && quanzi1 > 0 && quanzi2 != null
					&& quanzi2 > 0) {
	...
	public void setYwMarkId(String ywMarkId) {
		if (ywMarkId != null && ywMarkId.length() > 0) {
			for (int n = 10; n < 14; n++) {
				ywMarkId = ywMarkId.replaceAll(String.valueOf((char) n), "");
			}
		}
	...
	public static Set<ProcardTemplateChangeLogDetail> diffrentTwoPt(
			ProcardTemplate oldpt, ProcardTemplate newpt,
			ProcardTemplateChangeLog changeLog) {
		if (oldpt == null) {
			return null;
		}
		if (newpt == null) {
		...
		diffrentsx(logDetailSet, oldpt.getMarkId(), newpt.getMarkId(), "件号",
				newtime, user, changeLog, 0);
		diffrentsx(logDetailSet, oldpt.getYwMarkId(), newpt.getYwMarkId(),
				"业务件号", newtime, user, changeLog, 1);
		diffrentsx(logDetailSet, oldpt.getBanBenNumber(), newpt
				.getBanBenNumber(), "版本号", newtime, user, changeLog, 2);
		diffrentsx(logDetailSet, oldpt.getProName(), newpt.getProName(), "名称",
				newtime, user, changeLog, 3);
		diffrentsx(logDetailSet, oldpt.getSpecification(), newpt
				.getSpecification(), " 规格", newtime, user, changeLog, 4);
		diffrentsx(logDetailSet, oldpt.getCaizhi(), newpt.getCaizhi(), "材质",
				newtime, user, changeLog, 5);
		diffrentsx(logDetailSet, oldpt.getCorrCount(), newpt.getCorrCount(),
				"权值", newtime, user, changeLog, 6);
		diffrentsx(logDetailSet, oldpt.getQuanzi1(), newpt.getQuanzi1(), "权值1",
				newtime, user, changeLog, 6);
		diffrentsx(logDetailSet, oldpt.getQuanzi2(), newpt.getQuanzi2(), "权值2",
				newtime, user, changeLog, 6);
		diffrentsx(logDetailSet, oldpt.getUnit(), newpt.getUnit(), "单位",
				newtime, user, changeLog, 7);
		diffrentsx(logDetailSet, oldpt.getProcardStyle(), newpt
				.getProcardStyle(), "零件类型", newtime, user, changeLog, 8);
		diffrentsx(logDetailSet, oldpt.getWgType(), newpt.getWgType(), "物料类别",
				newtime, user, changeLog, 9);
		diffrentsx(logDetailSet, oldpt.getKgliao(), newpt.getKgliao(), "供料属性",
				newtime, user, changeLog, 10);
		diffrentsx(logDetailSet, oldpt.getZzjihuo(), newpt.getZzjihuo(),
				"自制件激活模式", newtime, user, changeLog, 11);
		diffrentsx(logDetailSet, oldpt.getJihuoType(), newpt.getJihuoType(),
				"激活类型", newtime, user, changeLog, 12);
		diffrentsx(logDetailSet, oldpt.getClType(), newpt.getClType(), "材料类型",
				newtime, user, changeLog, 13);
		diffrentsx(logDetailSet, oldpt.getBiaochu(), newpt.getBiaochu(), "表处",
				newtime, user, changeLog, 14);
		diffrentsx(logDetailSet, oldpt.getTuhao(), newpt.getTuhao(), "图号",
				newtime, user, changeLog, 15);
		diffrentsx(logDetailSet, oldpt.getShowSize(), newpt.getShowSize(),
				"尺寸", newtime, user, changeLog, 16);
		diffrentsx(logDetailSet, oldpt.getLingliaostatus(), newpt
				.getLingliaostatus(), "是否领料", newtime, user, changeLog, 17);
		diffrentsx(logDetailSet, oldpt.getNeedProcess(),
				newpt.getNeedProcess(), "外购件是否半成品", newtime, user, changeLog,
				18);
		diffrentsx(logDetailSet, oldpt.getLingliaoType(), newpt
				.getLingliaoType(), "领料方式", newtime, user, changeLog, 19);
		diffrentsx(logDetailSet, oldpt.getThisLength(), newpt.getThisLength(),
				"长", newtime, user, changeLog, 20);
		diffrentsx(logDetailSet, oldpt.getThisWidth(), newpt.getThisWidth(),
				"宽", newtime, user, changeLog, 21);
		diffrentsx(logDetailSet, oldpt.getThisHight(), newpt.getThisHight(),
				"高", newtime, user, changeLog, 22);
		diffrentsx(logDetailSet, oldpt.getCalculateType(), newpt
				.getCalculateType(), "计算方式", newtime, user, changeLog, 23);
		diffrentsx(logDetailSet, oldpt.getImportance(), newpt.getImportance(),
				"重要度", newtime, user, changeLog, 24);

		return logDetailSet;
	}

	public static ProcardTemplateChangeLogDetail diffrentsx(
			Set<ProcardTemplateChangeLogDetail> logDetailSet, String oldValue,
			String newValue, String sxName, String newtime, Users user,
			ProcardTemplateChangeLog changeLog, Integer xuhao) {
		if (!Util.isEquals(oldValue, newValue)) {
			ProcardTemplateChangeLogDetail detail = new ProcardTemplateChangeLogDetail();
	...
	public static ProcardTemplateChangeLogDetail diffrentsx(
			Set<ProcardTemplateChangeLogDetail> logDetailSet, Integer oldValue,
			Integer newValue, String sxName, String newtime, Users user,
			ProcardTemplateChangeLog changeLog, Integer xuhao) {
		if (!Util.isEquals(oldValue, newValue)) {
			ProcardTemplateChangeLogDetail detail = new ProcardTemplateChangeLogDetail();
			detail.setSxName(sxName);
	...
	public static ProcardTemplateChangeLogDetail diffrentsx(
			Set<ProcardTemplateChangeLogDetail> logDetailSet, Float oldValue,
			Float newValue, String sxName, String newtime, Users user,
			ProcardTemplateChangeLog changeLog, Integer xuhao) {
		if (!Util.isEquals(oldValue, newValue)) {
			ProcardTemplateChangeLogDetail detail = new ProcardTemplateChangeLogDetail();
			detail.setSxName(sxName);
	...
	public static ProcardTemplateChangeLogDetail diffrentsx(
			Set<ProcardTemplateChangeLogDetail> logDetailSet, Double oldValue,
			Double newValue, String sxName, String newtime, Users user,
			ProcardTemplateChangeLog changeLog, Integer xuhao) {
		if (!Util.isEquals(oldValue, newValue)) {
			ProcardTemplateChangeLogDetail detail = new ProcardTemplateChangeLogDetail();
			detail.setSxName(sxName);
