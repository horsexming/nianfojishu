...
public class CalendarServerImpl implements CalendarServer {
	...
	public List<Calendar> findCalendar(String start, String end) {
		...
		String hql = "from Calendar where start>?  and thingStatus = 'public' and calendarState<> '完成'" +
                " or ( start>?  and thingStatus = 'private' and calendarState<> '完成' and  addUserId=? )" +
                " or ( start>?  and thingStatus = 'private' and calendarState<> '完成' and  userId=?)";
		return totalDao.query(hql, start, start, loginUser.getId(),start,loginUser.getId());
	...
	public String addCalendars(Calendar calendar, String ids) {
		...
		for(Integer i=0;i<idss.length;i++){
			...
			if(idss[i]!=null&&idss[i].length()>0)
...
//			calendar.setAddDateTime(Util.getDateTime(null));// 添加时间
			...
			boolean bool2=Util.compareTime(startDate2, timeFormat, Util.getDateTime(timeFormat), timeFormat);
			if(!bool2){
				startDate2=startDate2.substring(0, 10)+ Util.getDateTime(" HH:mm:ss");
				calendar.setStart(startDate2);
			}
			// 结束时间
	...
	private Boolean addMessagePolling(Calendar calendar) throws ParseException {
		...
		if("成功".equals(messagePollingService.addMessage(m))){
            RtxUtil.sendNotify(m.getReceiveUsersCode(),m.getSendUsersName()+" 于："+m.getAddDate() +" 提醒您 \n"+ m.getMessageTitle()+"\n"+m.getMessage(),"系统消息","0","0");
			return true;
		};
		return false;
	}
...
//		Integer epId = CircuitRunServerImpl.createProcess(processName, OaAppDetail.class, detail.getId(), "detailStatus", "id", null);
		try {
			...
			if(calendar.getViewTime()==null){
				calendar.setViewTime(Util.getDateTime());
			}
			totalDao.update(calendar);
			totalDao.clear();
...
//				+ ") ";//本周的hql语句
	...
	@Override
	public List<Calendar> taskCalendar(String start) {
		List<Calendar> cList = totalDao.query("from Calendar  WHERE start<=? and endDate>=?",start,start);
		return cList;
	}

	@Override
	public List<Calendar> Calendarajax(String userName) {
		List<Calendar> cList = totalDao.query("FROM Calendar WHERE  userName=? AND calendarState='未完成'",userName);
		return cList;
	}

	@Override
	public Users tiemeing(Integer id) {
		Users users = new Users();
		String dateTime = Util.getDateTime("yyyy-MM-dd");
		List<FaceWorkTime> workList = totalDao.query("from FaceWorkTime where userId=? and dateTime =?", id,dateTime);
	...
	@Override
	public List<Calendar> findCalendargo(String start, String end) {
		List<Calendar> list=totalDao.query("from Calendar where start > '"+start+"' and endDate < '"+end+"' and thingStatus = 'public' and calendarState<> '完成'", null);
		return list;
	}
}
