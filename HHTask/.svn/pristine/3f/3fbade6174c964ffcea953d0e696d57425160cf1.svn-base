			...
			<div align="left">
				...
				<div style="" align="left">
					<div style="height: 100%;">
						...
						<input type="button" value="确认配件" onclick="confirmParts()" style="width:100px;height:50px;">
					</div>
				</div>
				<!-- 添加流水卡片模板操作 -->
				<div
					...
					<div id="selectDiv" style="display: none;" align="center">
						...
						<input id="module18" type="button" value="未绑工序外购件"
							onclick="window.open('ProcardTemplateAction!findwbdProcessWgProcard.action?id=${param.id}')"
						...
						<input id="module20" type="button" value="整体工序管理"
							onclick="window.open('procardTemplateGyAction_showBOMasExlAndProcedure.action?id=${param.id}')"
							style="width: 120px; height: 40px;" />
						...
						<form id="lingForm" style="margin: 0px; padding: 0px;">
							...
							<table class="table" style="width: 100%;">
								...
								<tr>
									...
									<td>
										<input name="procardTemplate.biaochu" id="zuhebiaochu">
									</td>
								</tr>
								<tr>
									<th align="right">
							...
							<table class="table" style="width: 100%;">
								...
								<tr>
									...
									<td>
										...
										<input type="hidden" value="" name="procardTemplate.isycl"
											id="wajisycl" />
										<font color="red">*</font>
								...
								<tr>
									...
									<td>
										<input type="text" id="waibili" name="procardTemplate.bili"
											readonly="readonly">
									</td>
								</tr>
								<tr>
...
function addDiyDom(treeId, treeNode) {
	...
	if (treeNode.proStruts == '外购') {
		editStr += '<div class="diy" style="text-align: right;width:80px;">'
				+ treeNode.quanzi2 / treeNode.quanzi1 + '</div>';
	} else {
		editStr += '<div class="diy" style="text-align: right;width:80px;">' + treeNode.corrCount + '</div>';
	}
	var bzStatusStr = '';
	...
	$
			.ajax( {
				...
				success : function(doc) {
					...
					$(doc)
							.each(function() {
								//var b = true;
									...
									var appendProName = "";
									appendProName = "<input type='checkbox' class='checkItem' value='"+$(this).attr('id')+"'>"+$(this).attr('proName');
									/*if(){
					...
					//添加表头
					var li_head = ' <li class="head"><a><div class="diy" >产品名称</div><div class="diy" align="center">件号/图号</div><div class="diy">产品类型</div>' +
					'<div class="diy" style="width:80px;">用量</div><div class="diy" style="width:60px;">编制状态</div><div class="diy"  style="width:30px;">版本' +
					'</div><div class="diy"  style="width:30px;">版次</div><div class="diy">编制人员</div></a></li>';
					var rows = $("#treeDemo").find('li');
					if (rows.length > 0) {
						rows.eq(0).before(li_head);
<%--						$("#treeDemo")--%>
<%--								.append(--%>
<%--										'<li ><input type="button" value="确认配件" onclick="confirmParts()"></li>');--%>
					} else {
...
function confirmParts(){
	...
	$(checkItem).each(function(){
		if($(this).is(":checked")){
			var procardId= $(this).val();
			$.ajax({
				async :false,
				type:"post",
				url:"${pageContext.request.contextPath}/ProcardTemplateAction!getProcardById.action",
				data:{
					id:procardId
				},
				dataType:"JSON",
				success:function(data){
					...
					for ( var i = 1; i < index - 1; i++) {
						var old_id = window.parent.document.getElementById("id_" + priceId);
						if (old_id != null && old_id.value == priceId) {
							bool = false;
						}else{
							var mark = $("#procardId1_"+procardId1).val();
							if(mark==markId){
								continue;
							}
							var aboutBh="";
		...
		<%--					unitPrice = unitPrice.toFixed(2);--%>
							...
							if(priceId!=null && priceId!=0){
								...
								var newtd5 = "<td>"+hsPrice+"<input type='hidden' id='hsprice_"+(index - 2)+"' value='"+hsPrice+"' name='mpList["+(index-2)+"].hsPrice' ></td>";
							}else{
							...
							var newtd1 = '<td>' + markId + '<input type="hidden" value='+ markId + ' name="pmList[' + (index - 2)
									+ '].pieceNumber" id="procardId1_'+procardId1+'"><input type="hidden" value=' + priceId
	...
	if(startIndex<index){
		window.parent.document.getElementById("sub").disabled=true;
...
$(document).ready(function(){
	...
	$
			.ajax( {
				type : "POST",
				url : "procardTemplateGyAction_getSpjdCount.action",
				dataType : "json",
				success : function(data) {
					if(data=="2"){
						$(".jdtr").hide();
...
function showCardTz(){
	var id=$("#cardId").val();
	$("#showProcess").attr("src",
			"procardTemplateGyAction_showCardTz.action?id=" + id+"&tag=${param.pageStatus}");
	chageDiv('block');
}
...
function processAndWgProcard(){
	if(window.confirm("是否一键关联工序外购件?")){
		...
		$
			.ajax( {
				type : "POST",
				url : "ProcardTemplateAction!processAndWgProcard.action",
				data : {
					id:${param.id}
				},
				dataType : "json",
...
function daoruson(){
	var id=$("#cardId").val();
	$("#showProcess").attr("src",
			"procardTemplateGyAction_todaoruson.action?id="+id);
	chageDiv('block');
}	
		</SCRIPT>
	</body>
