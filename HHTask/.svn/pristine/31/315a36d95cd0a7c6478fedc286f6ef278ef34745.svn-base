	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
				...
				<s:if test="tag=='wwPlan'">
				<form action="BaoXiaoDanAction!saveBaoXiaoDan1.action?tag=ww" method="post" 	onsubmit="return checkForm()">
				</s:if>
				<s:else>
					<form action="BaoXiaoDanAction!saveBaoXiaoDan1.action?tag=wg" method="post" 	onsubmit="return checkForm()">
				</s:else>
						...
						<tbody>
							...
							<s:if test="pageStatus!=null&&pageStatus!=''">
								<s:iterator value="list" id="pageList" status="pageStatus1">
									<tr>
									<th align="center">
									<input  name="listDetail[${pageStatus1.index}].goodsStoreMarkId" type="text" value="${pageList.markId}">
									<input  name="ids[${pageStatus1.index}]" type="hidden" value="${pageList.id}">
								</th>
								<s:if test="tag=='wwPlan'">
									<th align="center">
									 <input type="text"  name="listDetail[${pageStatus1.index}].processesNo" value="${pageList.processNo}">
								</th>
								</s:if>
								<th align="center">
									<input type="text" id="count_id${pageStatus1.index}" name="listDetail[${pageStatus1.index}].partName"  value="${pageList.proName}"/>
								</th>
								<th align="center">
									<input type="text" style="width: 280px;"   name="listDetail[${pageStatus1.index}].goodsStoreLot"  value="${pageList.selfCard}"/>
								</th>
								<th align="center">
									<input type="text" style="width: 280px;"   name="listDetail[${pageStatus1.index}].goodsStoreCount"  value="${pageList.number}"/>
								</th>
								<th align="center">
<%--									<input type="hidden" name="listDetail[${pageStatus1.index}].producestatus" value="${pageList.type}">--%>
								<select id="h${pageStatus1.index}" name="listDetail[${pageStatus1.index}].money"   onmouseover="funPrice(${pageStatus1.index},'${pageList.markId}')"  onclick="hejijine()" >
							...
							<tr>
							<th align="center">
									<input id="course0" name="listDetail[0].goodsStoreMarkId" type="text" value="${wwPlan.markId}">
									<input  name="ids[0]" type="hidden" value="${wwPlan.id}">
								</th>
								<s:if test="tag=='wwPlan'">
									<th align="center">
									 <input type="text"  name="listDetail[0].processesNo" value="${wwPlan.processNo}">
								</th>
								</s:if>
								<th align="center">
									<input type="text" id="count_id0" name="listDetail[0].partName"  value="${wwPlan.proName}"/>
								</th>
								<th align="center">
									<input type="text" style="width: 280px;"   name="listDetail[0].goodsStoreLot"  value="${wwPlan.selfCard}"/>
								</th>
								<th align="center">
									<input type="text" style="width: 280px;"   name="listDetail[0].goodsStoreCount"  value="${wwPlan.number}"/>
								</th>
								<th align="center">
<%--									<input type="text" name="listDetail[0].producestatus" value="${wwPlan.type}">--%>
								<select name="listDetail[0].money" onclick="hejiJine()" id="h0">
								<s:iterator value="list" id="pageList">
									<option value="${pageList.hsPrice}">${pageList.hsPrice}</option>	
								</s:iterator>
								</select>
								</th>
							</tr>
...
//查询对应价格
function funPrice(obj,markId){//下标及件号
		$.ajax( {
		type : "POST",
		url : "ProcardAction!findWaigouWaiweiPlanPrice",
		data : {
			markId : markId
		},
		dataType : 'json',
		success : function(data) {
			...
			$("#h" + obj).empty();
			$(data).each(function() {
							$( "<option value='" + this.hsPrice + "'> "+ this.hsPrice +"</option>").appendTo("#h" + obj);
				     	});
			hejiJine();
			}
		});
		$("#h" + obj).click(function(){
			hejiJine();
		})
}
...
function selectDept(few) {
	var id = "course" + few;
	createDept(id, "BaoXiaoDanAction!findjianhao");
	selectSubjects(few);
}
//选择批次
