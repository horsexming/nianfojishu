	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
				...
				<form
					action="paymentVoucherAction_findExamList.action?tag=<s:property value='tag' />"
					method="post">
					<table class="table">
						<tr>
						...
						<s:iterator value="list" status="se" id="bar">
							...
							<td>
							<s:if test="#bar.bargain_source=='OA'">
							OA
						</s:if>
						<s:if test="#bar.bargain_source=='SB'">
							设备维修
						</s:if>
						<s:if test="#bar.bargain_source=='KVP'">
							KVP
						</s:if>
						<s:if test="#bar.bargain_source=='QT'">
							其他
						</s:if>
							</td>
							<td>
...
<%--										<font color="red">合计<label id="allMoney"></label>元</font>--%>
	...
	<script type="text/javascript">
    	function chageType(obj,form){
    		if(obj.id=="ok"){
    			form.action="bargainAction_updateExamBargain.action?tag=ok";
    			form.submit();
    		}else if (obj.id=="ng"){
    			form.action="bargainAction_updateExamBargain.action?tag=ng";
    			form.submit();
    		}
    	}
