		...
		$
				.each(obj, function(i, procard) {
					...
					//判断激活状态
						...
						var banben = "";
						if (procard.banBenNumber != ""&&procard.banBenNumber != null) {
						...
						var cangqu = ""
						if(procard.procardStyle == '外购'){
							wgTypeandkuwei = "(<b><font color= 'red'>"+procard.wgType+"</font></b>)"+"(<b><font color= 'red'>"+procard.wlKw+"</font></b>)";
							cangqu = "&nbsp;&nbsp;(<b><font color= 'red'>"+procard.cangqu+"</font></b>)";
						}

						var nextWidthCon = "[<b>" + procard.procardStyle
								+ otherStatus + "</b>]" + flstatus
								+ procard.proName + "&nbsp;&nbsp;"+wgTypeandkuwei+"<br/>" + banben
								+ procard.markId + "(<b>" + tjNumber + "/"
								+ procard.filnalCount + "</b>)"+cangqu;
						var widthCon = "";
						if (width > 60) {
							color = "#ffffff";
		...
		<center>
			<div align="center" id="divWatermark"
				...
				<a href="javascript:;" onclick="javascript:location.reload(true)">
					[刷新]</a>
				<a href="javascript:;"
					onclick="javascript:location.href='<%=basePath%>/System/SOP/produce/Procard_gxjdck.jsp?id=${procard.id}'">[进入列表模式]</a>
				<br />
				(件号:${procard.markId}
				...
				<br/>
				<a href="ProcardAction!exportExcel.action?id=${procard.id}">导出</a>
			</div>
			<!-- 生产进度 -->
			<div id="viewProcard"
