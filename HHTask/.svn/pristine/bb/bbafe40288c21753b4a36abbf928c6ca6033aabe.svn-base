...
<html>
	...
	<body>
		...
		<div id="gongneng" style="width: 100%;">
			<div align="center">
				<div align="center">
				<h3>件号：${wwPlan.markId}&nbsp;&nbsp;&nbsp;工序号：${wwPlan.processNo}&nbsp;&nbsp;
					工序名称：${wwPlan.processName}&nbsp;&nbsp;</h3>
				</div>
				<br/>
				<div align="center">
					<table class="table">
						...
						<s:iterator value="priceList" id="pagePrice" status="pageStatus">
							...
							<td align="center">${pagePrice.gys}</td>
							...
							<td align="right">${pagePrice.wwType}</td>
							<td align="center">
								<s:if test="#pagePrice.flag=='had'">
									<font color="green">已选中</font>
								</s:if>
								<s:else>
							<input type="button" value="选择" onclick="updatePrice(${pagePrice.id},${wwPlan.id})">
								</s:else>
							</td>
						</s:iterator>
						<s:if test="priceList==null||priceList.size()==0">
						<tr>
							<td align="center" colspan="10"> <font color="red">没有找到待填充的合同，请前往添加合同!</font>
							</td>
						</tr> 
						</s:if>
					</table>
...
function  updatePrice(id,id2){
	if(window.confirm("您是否选定这份合同作为此外委的依据?")){
		window.location.href="ProcardAction!updatePrice2.action?cpage=${cpage}&id="+id+"&id2="+id2;
	}
}
</SCRIPT>
	</body>
