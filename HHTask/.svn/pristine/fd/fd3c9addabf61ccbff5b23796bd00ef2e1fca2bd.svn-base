...
public class ProcessTemplatesb  implements java.io.Serializable{
	private static final long serialVersionUID =1L;

	private Integer id;// 主键
	private Integer processtId;//工序BOM工序Id
	...
	/********** 对应关系 ********************/
	private ProcardTemplatesb procardTemplatesb;// 对应流水卡片
...
//	}
	...
	public Integer getProcesstId() {
		return processtId;
	}

	public void setProcesstId(Integer processtId) {
		this.processtId = processtId;
	}

	public static Set<ProcardTemplatesbChangeLogDetail> diffrentTwoPt(ProcessTemplatesb oldprocess,ProcessTemplatesb newprocess,ProcardTemplatesbChangeLog changeLog){
		if(oldprocess==null){
			return null;
		}
		if(newprocess==null){
		...
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessNO(), newprocess.getProcessNO(), "工序号", newtime, user, changeLog, 1);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessName(), newprocess.getProcessName(), "工序名", newtime, user, changeLog, 2);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessExplain(), newprocess.getProcessExplain(), "工序要求", newtime, user, changeLog, 3);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessStatus(), newprocess.getProcessStatus(), "并行状态", newtime, user, changeLog, 4);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getIsSpecial(), newprocess.getIsSpecial(), "特殊工序", newtime, user, changeLog, 5);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProductStyle(), newprocess.getProductStyle(), "生产类型", newtime, user, changeLog, 6);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getIsPrice(), newprocess.getIsPrice(), "是否参与奖金分配", newtime, user, changeLog, 7);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getZjStatus(), newprocess.getZjStatus(), "是否首检", newtime, user, changeLog, 8);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getShebeistatus(), newprocess.getShebeistatus(), "设备验证", newtime, user, changeLog, 9);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGongzhuangstatus(), newprocess.getGongzhuangstatus(), "工装验证", newtime, user, changeLog, 10);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getLiangjustatus(), newprocess.getLiangjustatus(), "量具验证", newtime, user, changeLog, 11);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGuifanstatus(), newprocess.getGuifanstatus(), "规范验证", newtime, user, changeLog, 12);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getKaoqingstatus(), newprocess.getKaoqingstatus(), "考勤验证", newtime, user, changeLog, 13);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getIsJisuan(), newprocess.getIsJisuan(), "是否计算", newtime, user, changeLog, 14);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getNeedSave(), newprocess.getNeedSave(), "是否转存", newtime, user, changeLog, 15);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getIsNeedFuliao(), newprocess.getIsNeedFuliao(), "是否需要辅料", newtime, user, changeLog, 16);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpcaozuojiepai(), newprocess.getOpcaozuojiepai(), "操作人工节拍", newtime, user, changeLog,17 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpshebeijiepai(), newprocess.getOpshebeijiepai(), "操作设备节拍", newtime, user, changeLog, 18);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzzhunbeijiepai(), newprocess.getGzzhunbeijiepai(), "准备过程人工节拍", newtime, user, changeLog, 19);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzzhunbeicishu(), newprocess.getGzzhunbeicishu(), "准备次数", newtime, user, changeLog,20 );
		
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOptechnologyRate(), newprocess.getOptechnologyRate(), "技能指数", newtime, user, changeLog,21 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpCouldReplaceRate(), newprocess.getOpCouldReplaceRate(), "可替换人数", newtime, user, changeLog,22 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpfuheRate(), newprocess.getOpfuheRate(), "负荷指数", newtime, user, changeLog,23 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpnoReplaceRate(), newprocess.getOpnoReplaceRate(), "操作不可替换系数", newtime, user, changeLog, 24);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpzonghezhishu(), newprocess.getOpzonghezhishu(), "操作综合指数", newtime, user, changeLog,25 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getOpzongheqiangdu(), newprocess.getOpzongheqiangdu(), "操作综合强度", newtime, user, changeLog,26 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGztechnologyRate(), newprocess.getGztechnologyRate(), "技术指数", newtime, user, changeLog, 27);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzCouldReplaceRate(), newprocess.getGzCouldReplaceRate(), "可替换人数", newtime, user, changeLog,28 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzfuheRate(), newprocess.getGzfuheRate(), "负荷指数", newtime, user, changeLog, 29);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGznoReplaceRate(), newprocess.getGznoReplaceRate(), "准备过程不可替换系数", newtime, user, changeLog, 30);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzzonghezhishu(), newprocess.getGzzonghezhishu(), "准备过程综合指数", newtime, user, changeLog, 31);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGzzongheqiangdu(), newprocess.getGzzongheqiangdu(), "综合强度", newtime, user, changeLog, 32);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGuding(), newprocess.getGuding(), "固定节拍", newtime, user, changeLog, 33);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessMomey(), newprocess.getProcessMomey(), "工序单价", newtime, user, changeLog, 34);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcesdianshu(), newprocess.getProcesdianshu(), "计件点数", newtime, user, changeLog, 35);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProcessjjMoney(), newprocess.getProcessjjMoney(), "工序计件单价", newtime, user, changeLog, 36);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getJjratio(), newprocess.getJjratio(), "计件单价系数", newtime, user, changeLog, 37);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getMeasuringMatetag(), newprocess.getMeasuringMatetag(), "量具名称", newtime, user, changeLog,38 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getMatetag(), newprocess.getMatetag(), "工装名称", newtime, user, changeLog,39);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getAllJiepai(), newprocess.getAllJiepai(), "总节拍", newtime, user, changeLog,40);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getCapacity(), newprocess.getCapacity(), "单班时长", newtime, user, changeLog,41 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getCapacitySurplus(), newprocess.getCapacitySurplus(), "产能盈余", newtime, user, changeLog,42 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getCapacityRatio(), newprocess.getCapacityRatio(), "产能比", newtime, user, changeLog,43 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getAlldeliveryDuration(), newprocess.getAlldeliveryDuration(), "总延误时长", newtime, user, changeLog,44 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getDeliveryDuration(), newprocess.getDeliveryDuration(), "配送时长", newtime, user, changeLog,45 );
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getDeliveryRatio(), newprocess.getDeliveryRatio(), "配送比", newtime, user, changeLog, 46);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getDeliveryPeriod(), newprocess.getDeliveryPeriod(), "配送周期", newtime, user, changeLog, 47);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getDeliveryAmount(), newprocess.getDeliveryAmount(), "送货量", newtime, user, changeLog, 48);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getProSingleDuration(), newprocess.getProSingleDuration(), "总成单班生产时长", newtime, user, changeLog, 49);
		ProcardTemplatesb.diffrentsx(logDetailSet, oldprocess.getGys(), newprocess.getGys(), "供应商", newtime, user, changeLog, 50);
		return logDetailSet;
	}
