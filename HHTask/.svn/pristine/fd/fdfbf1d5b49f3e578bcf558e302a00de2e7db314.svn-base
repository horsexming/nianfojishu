					...
					<table class="table">
						...
						<s:iterator value="procardList" id="pageProcard"
							...
							<td width="12%" align="center">
								<font size="4">${pageProcard.markId}</font><font color="red">(${pageProcard.status})</font>
							</td>
							...
							<td width="45%" align="center" colspan="4">
								<table class="table"
									id="processwwTb<s:property value="#pageProcard.id" />"
									width="100%">
									<s:if test="#pageProcard.wwApplyDetailList.size()>0">
							...
							<td width="8%" align="center" id="yxxq<s:property value="#pageProcard.id"/>">
							...
							<s:if
								...
								<form id="yxForm<s:property value="#pageindex.index" />" method="post">
									<tr id="processTr<s:property value="#pageProcard.id" />"
										bgcolor="#f1f1f1">
										<td>
												...
												<s:iterator value="#pageProcard.processList"
													...
													<td>
														<input type="checkbox"
															...
															<input type="hidden" id="wwmaxCount_<s:property value="#pageindex.index" />_<s:property value="#processStatus.index" />" value="${pageProcess.applyWwCount}">
										...
										<table id="processtb<s:property value="#pageindex.index" />" style="display: none"  class="table" width="100%;" border="0" cellpadding="0" cellspacing="0">
											<tr><th>数量</th><td>
											<input type="hidden" id="procardfilnalCount<s:property value="#pageindex.index" />" value="${pageProcard.filnalCount}"/>
											<input id="applyCount<s:property value="#pageindex.index" />" name="pwwApplyDetailList[0].applyCount"  onkeyup="mustBeNumber('applyCount')" value="${pageProcard.filnalCount}"/> </td>
											</tr>
											<tr><th>外委方式</th><td><input name="pwwApplyDetailList[0].wwType" id="wwtypea<s:property value="#pageindex.index" />" value="工序外委" type="radio" onclick="selectwwtype('工序外委')"/>工序外委
												<input name="pwwApplyDetailList[0].wwType" id="wwtypeb<s:property value="#pageindex.index" />" value="包工包料" type="radio" onclick="selectwwtype('包工包料')" checked="checked"/>包工包料 </td>
											</tr>
										...
										<td align="center" id="yxop<s:property value="#pageindex.index"/>">
										<input type="button" value="预选" id="yuxuanbtn<s:property value="#pageindex.index" />"
										onclick="submityx(<s:property value="#pageindex.index" />,${pageProcard.id})">
...
function cbAll(index){
	...
	var maxwwcount=$("#procardfilnalCount"+index).val();
			...
			if(checkAll.checked==true){
				...
				for(var i=0;i<checkboxs.length;i++){
					...
					var nowmaxwwcount = $("#wwmaxCount_"+index+"_"+i).val();
					if((maxwwcount-nowmaxwwcount)>0){
						maxwwcount = nowmaxwwcount;
					}
				}
			...
			$("#applyCount"+index).val(maxwwcount);
}
...
function submityx(index,procardId){
	...
	$.ajax( {
		...
		success : function(data) {
			if (data.success=="true"||data.success) {
				alert("预选成功");
				var n=0;
				var html="<tr bgcolor='yellow'>" +
				"<td width='22%'>"+data.data1.processNOs+"</td>" +
				"<td width='33%' align='left'>"+data.data1.processNames+"</td>" +
				"<td width='22%'>"+data.data1.applyCount+"</td>" +
				"<td width='22%'>"+data.data1.wwType+"</td>" +
				"</tr>"
				$("#processwwTb"+procardId).append(html);
				var wwtype="包公包料";
				if($("#wwtypea"+index).attr("checked")){
					wwtype="工序委外";
				}
				
				$(data.data2).each(function() {
					$("#processTr"+data.data2[n]).hide();
					$("#yxxq"+data.data2[n]).empty();
					$("#yxxq"+data.data2[n]).html("<font color='gray'>已被连带"+wwtype+"</font>");
					n++;
				})
