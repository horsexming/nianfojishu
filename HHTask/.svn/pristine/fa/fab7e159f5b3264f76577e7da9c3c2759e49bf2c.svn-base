		...
		<div class="container">
			<form action="productEBAction!updatePebUsers.action" method="post"  id="myform" autocomplete=off >
				...
				<div class="form-group row" id="appendBorrow">
					<label for="">借入信息&nbsp;&nbsp;<s:if test="tag!=null&&tag=='addLog'"><a onclick="addBorrowLog()">增加</a></s:if></label>
					<br><br>
								...
								<div class="form-group col-lg-5">
				                    <div class="input-group">
				                        ...
				                        <input class="form-control" type="text" name="pebUsers.borrowLogList[${ps.index}].borrowBanzu" 
				                        	value="${log.borrowBanzu}" class="borrowBanzu_${ps.index }" readonly="readonly">
				                    </div>
				                </div>
								<div class="form-group col-lg-5">
				                    <div class="input-group">
				                        ...
				                        <input class="form-control" type="text" name="pebUsers.borrowLogList[${ps.index }].borrowNum" 
				                        	value="${log.borrowNum}" onchange="compluteActualNum()" id="borrowNum_${ps.index}" readonly="readonly">
				                    </div>
				                </div>
				                <s:if test="tag!=null&&tag=='addLog'">
					                <div class="form-group col-lg-2"> 
					                	<!-- 记录LogId -->
										<input type="hidden" value="${log.id}" id="borrowLogId_${ps.index }" >
				...
				<div class="form-group row" id="appendLend">
					<label for="">借出信息&nbsp;&nbsp;<s:if test="tag!=null&&tag=='addLog'"><a onclick="addLendLog()">增加</a></s:if></label>
					<br><br>
								...
								<div class="form-group col-lg-5">
				                    <div class="input-group">
				                        ...
				                        <input class="form-control" type="text" name="pebUsers.lendLogList[${ps.index}].lendBanzu" 
				                        	value="${log.lendBanzu}" class="lendBanzu_${ps.index }" readonly="readonly">
				                    </div>
				                </div>
								<div class="form-group col-lg-5">
				                    <div class="input-group">
				                        ...
				                        <input class="form-control" type="text" name="pebUsers.lendLogList[${ps.index }].lendNum" 
				                        	value="${log.lendNum}" onchange="compluteActualNum()" id="lendNum_${ps.index}" readonly="readonly">
				                    </div>
				                </div>
				                <s:if test="tag!=null&&tag=='addLog'">
					                <div class="form-group col-lg-2"> 
										<!-- 记录LogId -->
										<input type="hidden" value="${log.id}" id="lendLogId_${ps.index }" >
				...
				<input type="hidden" value="${pebUsers.category}" name="pebUsers.category">
				<input type="hidden" value="${pebUsers.mbTarget}" name="pebUsers.mbTarget">
...
// 		alert(borrowDiv);
	...
	function delLine(type,index){
		if(confirm("确定要删除吗？")){
			var flag = true;//代表是新添加的，不用后台删除
			var logIdObj = null
			if(type=='borrow'){
				logIdObj= $("#borrowLogId"+index);
			}else if(type=='lend'){
				logIdObj= $("#lendLogId"+index);
			}
			if(logIdObj!=null && logIdObj!=""){
				var logId = logIdObj.val();
				if(logId!=null&& logId>0){
					var id = parseInt(logId);
					flag = false;
					$.ajax({
						type:"post",
						url:"productEBAction!deleteBorrowLog.action",
						dataType:"json",
						data:{
							"id":id,
							"pageStatus":"delLogAjax"
