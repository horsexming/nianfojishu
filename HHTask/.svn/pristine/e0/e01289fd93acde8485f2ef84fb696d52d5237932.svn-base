...
function uploadProcessDataAffixResend(data) {
	...
	<body style="text-align: center;">
		<div align="center" style="width: 900px;">
			<table class="table">
				<tr>
					<td style="width: 20%; font-size: 20px; font-weight: bold">
						工艺规程文件
					</td>
					<td style="width: 60%;">
						<form
							action="gongyiGuichengAction!uploadProcessDataAffix2.action?affixType=processImg"
							enctype="multipart/form-data" method="post">
							<div style="position: relative; display: none;">
								<input type="hidden" name="gongyiGuichengAffix.gongyiGuichengId"
									value="${gongyiGuichengAffix.gongyiGuichengId}" />
								<input type="hidden" name="gongyiGuichengAffix.processDataId"
									value="${gongyiGuichengAffix.processDataId}" />
								<!-- 
					        <iframe name="hidden_iframe" id="hidden_iframe"></iframe>   -->
							</div>
							<input type="file" name="processDataImgFile" />
							<select id="weizhi" name="gongyiGuichengAffix.weizhi">
								<option value="gygcsy">
									首页
								</option>
								<option value="gycxtb">
							...
							<input type="button" id="upload" value=" 上 传 "
								onclick="if(window.confirm('确认要上传吗?')){this.form.submit();}" />
						</form>
					</td>
					<td style="width: 20%;">
						&nbsp;
			...
			<table style="width: 100%; text-align: center; border: 0px;">
				<tr>
					<input type="button" id="finishButton" value="完成工序编辑" class="input"
						style="width: 120px;" />
					</td>
				</tr>
			</table>
...
<script type="text/javascript">
$(function() {
	var gongyiGuichengId = $(
			"input[name='gongyiGuichengAffix.gongyiGuichengId']").val();
	var processDataId = $("input[name='gongyiGuichengAffix.processDataId']")
			.val();
	$
			.ajax( {
				type : "post",
				...
				data : {
					...
					'gongyiGuichengAffix.weizhi' : 'gygcsy'
				},
				async : false,
				success : function(data) {
					...
					if (success) {
						...
						$(gongyiGuichengAffixList)
								.each(
										function(i, n) {
											var processDataId = this.processDataId == null ? ''
													: this.processDataId;
											var gongyiGuichengId = this.gongyiGuichengId == null ? ''
													: this.gongyiGuichengId;
											$(
													"<tr><td></td><td>"
															+ this.fileName
															+ "</td><td><a href='gongyiGuichengAction!downloadProcessDataAffix2.action?gongyiGuichengAffix.id="
															+ this.id
															+ "'>下载</a>&nbsp;&nbsp;<a href='gongyiGuichengAction!deleteGongyiGuichengAffix.action?gongyiGuichengAffix.id="
															+ this.id
															+ "&gongyiGuichengAffix.gongyiGuichengId="
															+ gongyiGuichengId
															+ "&gongyiGuichengAffix.processDataId="
															+ processDataId
															+ "' onclick='return window.confirm(\"确认要删除该条记录?\");'>删除</a></td></tr>")
													.insertAfter("#fujian");
										});
					}
				}
	...
	$
			.ajax( {
				...
				data : {
					...
					'gongyiGuichengAffix.processDataId' : processDataId,
					'gongyiGuichengAffix.affixType' : 'tupian',
					'gongyiGuichengAffix.weizhi' : 'gycxtb'
				},
				async : false,
				success : function(data) {
					var success = data.success;
	...
	if (processDataId) {
		...
		$(
				'<option value="shipin">视频</option><option value="tupian">图片</option>')
				.appendTo("#affixType");
	} else {
		$("#shipin").css("display", "none");
		$(
				'<option value="fujian">附件</option><option value="tupian">图片</option>')
				.appendTo("#affixType");
	...
	$("#affixType")
			.bind(
					"change",
					function() {
						var affixType = $(this).val();
						$("#weizhi").empty();
						if ('fujian' == affixType) {
							$('<option value="gygcsy">首页</option>').appendTo(
									"#weizhi");
						} else {
							$(
									'<option value="gygcsy">首页</option><option value="gycxtb">工艺程序图表</option>')
									.appendTo("#weizhi");
						}
					});
