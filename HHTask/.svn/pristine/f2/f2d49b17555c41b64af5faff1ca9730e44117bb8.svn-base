				...
				<div id="rootTemplateDiv">
					...
					<table class="table">
						...
						<s:iterator value="procardTemplateList" id="pageProcardTem"
							...
							<td> 
								<input type="text" name="carStyle${pageProcardTem.id}" id="carStyle${pageProcardTem.id}"  value="${pageProcardTem.carStyle}" disabled="disabled"><br/>
							</td>
								...
								<div align="right">
									<div id="manage${pageindex.index}"
										...
										<div style="float: left; margin: 0xp; padding: 0px;">
											<input type="button" value="修改车型(型号)" id="updateAreaBtn${pageProcardTem.id}" onclick="updateArea(${pageProcardTem.id})"/>
...
function updateArea(procardId){
	$("#carStyle"+procardId).removeAttr("disabled");
...
function  submitUpdate(procardId){
	var carObj=document.getElementById("carStyle"+procardId);
	var carStyle=$("#carStyle"+procardId).val().trim();
	if(carStyle==""){
		alert("输入不能为空,谢谢!");
		$("#carStyle"+procardId).html("");
		$("#carStyle"+procardId).focus();
		return ;
	}
	 //修改	
	 $.ajax( {
			type : "POST",
			url : "ProcardTemplateAction!ajaxUpdateCarStyle.action",
			data : {
				id:procardId,
				carStyle:carStyle,
			...
			dataType : "json",
			success : function(data) {				
				$("#carStyle"+procardId).attr("disabled","disabled");
