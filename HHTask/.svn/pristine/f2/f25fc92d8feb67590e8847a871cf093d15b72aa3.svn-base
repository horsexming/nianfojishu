...
.innerTb {
	...
	<body>
		<center>
			...
			<form action="procardTemplateSbAction_addProcessTemplate.action"
				method="post" onsubmit="return checkForm();">
				<input type="hidden" name="id" value="${id}" id="procardTemId">
				<h2>
					工序信息
				</h2>
				...
				<div align="left">
					<font color="red">如需插入工序,将会与此工序号和次工序号之后的工序顺延，列:插入2工序,之前的的2,3,4...工序号变为3,4,5...</font>
				</div>
				<table class="table" id="tb">
					<tr>
						...
						<td>
							<select name="processTemplatesb.productStyle" id="productStyle">
								<option >
									自制
								</option>
								<option >
					...
					<tr>
						...
						<td>
							<s:if test="procardTemplatesb.productStyle=='试制'">
							<select id="zjStatus" name="processTemplatesb.zjStatus" >
								<option value="no" >
									no
								</option>
								<option value="yes" >
							...
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<a onclick="changeYesOrNo()"><label id="allYesOrNo">
									一键全否
								</label> </a>
						</td>
						<th align="right">
						...
						<td>
							<select name="processTemplatesb.isPrice" id="isPrice">
								<option value="yes">
									参与
								</option>
								<option value="no">
					...
					<tr>
						...
						<td>
							<select name="processTemplatesb.kaoqingstatus" class="yesOrNo" id="kaoqingstatus">
								<option value="是">
									是
								</option>
								<option value="否">
						...
						<td>
							<select name="processTemplatesb.guifanstatus" id="guifanstatus"> 
								<option value="是">
									是
								</option>
								<option value="否">
						...
						<td>
							<select name="processTemplatesb.isJisuan" class="yesOrNo2" id="isJisuan">
								<option value="yes">
									是
								</option>
								<option value="no">
					...
					<tr>
						...
						<td>
							<select name="processTemplatesb.gongzhuangstatus" class="yesOrNo" id="gongzhuangstatus">
								<option value="是">
									是
								</option>
								<option value="否">
					...
					<tr>
						...
						<td>
							<select name="processTemplatesb.liangjustatus" class="yesOrNo" id="liangjustatus">
								<option value="是">
									是
								</option>
								<option value="否">
					...
					<tr>
						...
						<td>
							<select name="processTemplatesb.shebeistatus" class="yesOrNo" id="shebeistatus">
								<option value="是">
									是
								</option>
								<option value="否">
					...
					<tr>
						...
						<td>
							<select name="processTemplatesb.guding" class="yesOrNo">
								<option value="是">
									是
								</option>
								<option value="否">
					...
					<tr>
						<td colspan="6" align="center">
							...
							<s:if test="processTemplatesb.isNeedFuliao=='yes'">
								<input type="radio" name='processTemplatesb.isNeedFuliao'
								...
								<table id="fuliaoTb">
									...
									<s:else>
										<table id="fuliaoTb" style="display: none;">
											...
											<s:iterator value="processTemplatesb.processFuLiaoTemplate"
												id="pagefuliao" status="index">
												<tr id='fuliaoTr<s:property value="#index.index"/>'>
													<td style="border-top: 0px; border-left: 0px"
														align="center">
														<input type="hidden"
															name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].id"
															value="<s:property value="#pagefuliao.id"/>">
														<SELECT id="type<s:property value="#index.index"/>"
															name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].type">
															<option>
																<s:property value="#pagefuliao.type" />
															</option>
															<option>
																备件
															</option>
														...
														<td style="border-top: 0px; border-left: 0px"
															...
															<input type="text"
																id="shortnamefl<s:property value="#index.index"/>"
																onkeyup="getAllNamefls(<s:property value="#index.index"/>)"
																style="height: 20px;"
																onFocus="init('shortnamefl<s:property value="#index.index"/>','showAllfl<s:property value="#index.index"/>',<s:property value="#index.index"/>)"
																onBlur="hidediv('showAllfl<s:property value="#index.index"/>')"
																name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].name"
																value="<s:property value="#pagefuliao.name"/>" />
														</td>
														<td style="border-top: 0px; border-left: 0px"
															align="center">
															<input
																id="specification<s:property value="#index.index"/>"
																name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].specification"
																value="<s:property value="#pagefuliao.specification"/>">
														</td>

														<td style="border-top: 0px; border-left: 0px"
															align="center">
															<input id="unit<s:property value="#index.index"/>"
																name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].unit"
																value="<s:property value="#pagefuliao.unit"/>">
														</td>

														<td style="border-top: 0px; border-left: 0px"
															align="center">
															<input id="quanzhi1<s:property value="#index.index"/>"
																name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].quanzhi1"
																style="width: 80"
																value="<s:property value="#pagefuliao.quanzhi1"/>">
															:
															<input id="quanzhi2<s:property value="#index.index"/>"
																name="processTemplatesb.fuliaoList[<s:property value="#index.index"/>].quanzhi2"
																style="width: 80"
																value="<s:property value="#pagefuliao.quanzhi2"/>">
														</td>
														<td
															style="border-top: 0px; border-left: 0px; border-right: 0px"
															align="center">
										...
										<tr>
											<td colspan="6" align="center">
												<input id="addBtn" type="submit" class="input" value="添加" />
											</td>
										</tr>
								</table>
								</form>
...
function checkForm(){
	...
	var procardTemId = $("#procardTemId").val();
	var obj = null;
	$.ajax({
		type:"post",
	...
	if(obj!=null && obj.length>0){
  	for(var i=0;i<obj.length;i++){
  		var data = obj[i];
  		var oldNo = data.processNO;
	  		if(processNO==oldNo){
	  			if(confirm("您要添加的"+oldNo+"工序号已经存在，是否要继续添加?如继续添加则会将后续工序自动向后累加!")){
	  				return true;
	  			}else{
	   			return false;
	  			}
...
function findProcessByName(processName){
	$.ajax( {
		...
		success : function(pro) {
			...
			var productStyle= "${procardTemplatesb.productStyle}";
			if(productStyle=="试制"){
				$("#zjStatus").val("no");
			}else{
...
function addFuliaoLine() {
	...
	var html = "<tr id='fuliaoTr"
			...
			+ "<SELECT id='type"
			+ maxFuLiaoIndex
			+ "' name='processTemplatesb.fuliaoList["
			+ maxFuLiaoIndex
			...
			+ "<td style='border-top: 0px;border-left: 0px' align='center'> <div id='showAllfl"
			+ maxFuLiaoIndex
			+ "' style='background-color: #ffffff; position: absolute; visibility: hidden; z-index: 1;'> </div>"
			+ "<input type='text' id='shortnamefl"
			+ maxFuLiaoIndex
			+ "' onkeyup='getAllNamefls("
			+ maxFuLiaoIndex
			+ ")' style='height: 20px;' onFocus='init(\"shortnamefl"
			+ maxFuLiaoIndex
			+ "\",\"showAllfl"
			+ maxFuLiaoIndex
			+ "\","
			+ maxFuLiaoIndex
			+ ")' onBlur='hidediv(\"showAllfl"
			+ maxFuLiaoIndex
			+ "\")' name='processTemplatesb.fuliaoList["
			+ maxFuLiaoIndex
			+ "].name' /> </td>"
			...
			$(data).each(
					function() {
						var name = $(this).attr('name').replace(
								$("#shortnamefl" + index).val(),
								"<font color='red'>"
										+ $("#shortnamefl" + index).val()
										+ "</font>");
						$("#showAllfl" + index).append(
								"<div onmouseover='ondiv(this)' onmouseout='outdiv(this,shortnamefl"
										+ index
										+ ")' onclick='selectdivfl(this,"
...
var yesOrNo2 = "no";
//一键全是或全否
function changeYesOrNo() {
	$(".yesOrNo option[value='" + yesOrNo + "']").attr("selected", "selected");
	$(".yesOrNo2 option[value='" + yesOrNo2 + "']")
