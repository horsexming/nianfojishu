...
<%--								<input type="button" id="submitBtn" value="添加"--%>
...
$("#submitBtn").click(function() {
				...
				for ( var i = 0; i < treeNode.length; i++) {
				 ...
				 parent.$('#fid').val(id);
				 parent.$('#tishiPerson').val(name);
   				parent.layer.tips('可以查看的人员', '#tishiPerson', {time: 5000});
...
parent.mfzTree = function() {
	$.ajax( {
		url : 'DeptNumberAction!findAlldeptforhege.action',
		type : 'post',
		dataType : 'json',
		async: false,
		cache : true,
