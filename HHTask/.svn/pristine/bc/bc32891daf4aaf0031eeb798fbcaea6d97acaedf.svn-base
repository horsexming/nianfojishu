...
public class HttpRestClientServerImpl implements HttpRestClientServer {
	 ...
	 * @throws IOException
	 */
	public Object[] getList(Integer pageIndex, Integer pageSize, OrderForB2B pageOrderForB2B, String fbStartDate,
			String fbEndDate, String cnStartDate, String cnEndDate) throws IOException {
		String poStatus = "all";
		...
		String LIST_URL = "https://openapi.huawei.com:443/service/esupplier/findPoLineList/1.0.0/20/" + pageIndex;
		// String LIST_URL =
		...
		sb.append("{\"poStatus\":\"" + poStatus + "\",\"shipmentStatus\":\"" + shipmentStatus
				+ "\",\"statusType\":\"COL_TASK_STATUS\",\"colTaskOrPoStatus\":\"" + colTaskOrPoStatus
				+ "\",\"poSubType\":\"" + poSubType + "\",\"itemCode\":\"" + itemCode + "\",\"poNumber\":\"" + poNumber
				+ "\",\"publishDateStart\":\"" + fbStartDate + "\",\"publishDateEnd\":\"" + fbEndDate
				+ "\",\"promiseDateStart\":\"" + cnStartDate + "\",\"promiseDateEnd\":\"" + cnEndDate + "\"}");
		...
		while ((line = reader.readLine()) != null) {
			...
			for (JsonElement jsonElement : jsonArray) {
				OrderForB2B orderForB2B = gson.fromJson(jsonElement, new TypeToken<OrderForB2B>() {
				}.getType());
				String publishDate = Util.DateToString(
						Util.StringToDate(orderForB2B.getPublishDate(), "yyyy-MM-dd'T'HH:mm:ss.000+0800"),
						"yyyy-MM-dd HH:mm:ss");
				String promiseDate = Util.DateToString(
						Util.StringToDate(orderForB2B.getPromiseDate(), "yyyy-MM-dd'T'HH:mm:ss.000+0800"),
						"yyyy-MM-dd HH:mm:ss");
	...
	public OrderForB2B findPOById(OrderForB2B pageOrderForB2B) {
		try {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("{\"instanceId\":\"1\",\"poHeaderId\":\"" + pageOrderForB2B.getPoHeaderId()
					+ "\",\"poReleaseId\":\"" + pageOrderForB2B.getPoReleaseId()
			...
			while ((line = reader.readLine()) != null) {
				...
				Gson gson = new Gson();
				for (JsonElement jsonElement : jsonArray) {
					orderForB2B = gson.fromJson(jsonElement, new TypeToken<OrderForB2B>() {
					}.getType());
					String publishDate = Util.DateToString(
							Util.StringToDate(orderForB2B.getPublishDate(), "yyyy-MM-dd'T'HH:mm:ss.000+0800"),
							"yyyy-MM-dd HH:mm:ss");
					String promiseDate = Util.DateToString(
							Util.StringToDate(orderForB2B.getPromiseDate(), "yyyy-MM-dd'T'HH:mm:ss.000+0800"),
							"yyyy-MM-dd");
					String needByDate = Util.DateToString(
							Util.StringToDate(orderForB2B.getNeedByDate(), "yyyy-MM-dd'T'HH:mm:ss.000+0800"),
							"yyyy-MM-dd");
					orderForB2B.setPublishDate(publishDate);
					orderForB2B.setPromiseDate(promiseDate);
	...
	public String addPrice(OrderForB2B pageOrderForB2B) {
		...
		if (dbOrderForB2B != null) {
			// 查询价格信息
			String hql_price = "from Price where partNumber=? and pricePeriodStart<'" + Util.getDateTime()
					+ "' and (pricePeriodEnd>'" + Util.getDateTime()
					+ "' or pricePeriodEnd is null or pricePeriodEnd='') ";
			if (dbOrderForB2B.getItemRevision() != null && dbOrderForB2B.getItemRevision().length() > 0) {
				hql_price += " and banbenhao='" + dbOrderForB2B.getItemRevision() + "' ";
			}
			Price price = (Price) totalDao.getObjectByCondition(hql_price, dbOrderForB2B.getItemCode());
			// if (price == null) {
			price = new Price();
			...
			ClientManagement cl = (ClientManagement) totalDao.getObjectByCondition(hql_cliet);
			...
			price1.setTaxprice(dbOrderForB2B.getTaxRate().doubleValue() * 100);
			price1.setFirstnum(0);
			price1.setEndnum(0);
			price1.setHsPrice(dbOrderForB2B.getPriceOverride().doubleValue() * (1 + dbOrderForB2B.getTaxRate()));
			price1.setBhsPrice(dbOrderForB2B.getPriceOverride().doubleValue());
			priceList.add(price1);
			return priceServerDao.addPrice(priceList, price, null, null, "B2BAdd", null);
			// }
		}
		return "未查到订单信息!请重试!";
	}
	 ...
	 */
	@Override
	public String signBackPOList(OrderForB2B pageOrderForB2B, String operateType) throws IOException {
		String mess = "处理失败!";
		OrderForB2B dbOrderForB2B = findPOById(pageOrderForB2B);
		if (dbOrderForB2B != null) {
			...
			StringBuilder sb = new StringBuilder();
			sb.append("{\"operateType\":\"" + operateType + "\",\"colTaskQueries\":[{\"lineLocationId\": "
					+ dbOrderForB2B.getLineLocationId() + ",\"instanceId\": " + dbOrderForB2B.getInstanceId()
					+ ",\"poLineId\": " + dbOrderForB2B.getPoLineId() + ",\"businessType\":\"new_po\"}]}");
			...
			while ((line = reader.readLine()) != null) {
				...
				if ("success".equals(code)) {
					if ("accept".equals(operateType)) {
						...
						OrderManager oldOrderManager = (OrderManager) totalDao.getObjectByCondition(hql_order,
								dbOrderForB2B.getPoHeaderId(), dbOrderForB2B.getPoReleaseId());
						if (oldOrderManager == null) {
							// 将华为订单转换为PEBS内部订单 dbOrderForB2B
							OrderManager outOrderManager = new OrderManager();
							outOrderManager.setPoHeaderId(dbOrderForB2B.getPoHeaderId());
							outOrderManager.setPoReleaseId(dbOrderForB2B.getPoReleaseId());
							outOrderManager.setOutOrderNumber(dbOrderForB2B.getPoNumber());
							...
							outOrderManager.setPaymentDate(dbOrderForB2B.getPromiseDate());
							String hql_cliet = "from ClientManagement where clientcompanyname='华为技术有限公司'";
							ClientManagement cl = (ClientManagement) totalDao.getObjectByCondition(hql_cliet);
							outOrderManager.setCustome(cl);
							outOrderManager.setClientName(cl.getClientcompanyname());
							outOrderManager.setClientDz(cl.getClientdz());
							outOrderManager.setClientFzr(dbOrderForB2B.getAgentName());
							outOrderManager.setDeliveryStatus("否");
							outOrderManager.setDocumentaryPeople(Util.getLoginUser().getName());
							outOrderManager.setDocumentaryPeopleId(Util.getLoginUser().getId());
							outOrderManager.setBillingPeople(Util.getLoginUser().getName());
							outOrderManager.setAddTime(Util.getDateTime());

							List<ProductManager> pmList = new ArrayList<ProductManager>();
							ProductManager productManager = new ProductManager();
							productManager.setName(dbOrderForB2B.getItemDescription());
							productManager.setPieceNumber(dbOrderForB2B.getItemCode());
							productManager.setYwMarkId(dbOrderForB2B.getItemCode());
							productManager.setNum(dbOrderForB2B.getQuantity());
							productManager.setUnitPrice(dbOrderForB2B.getTotalAmount().doubleValue());
							productManager.setRemark(dbOrderForB2B.getRemark());
							productManager.setUnit(
									dbOrderForB2B.getPriceOverride().doubleValue() * (1 + dbOrderForB2B.getTaxRate()));
							productManager.setBhsPrice(dbOrderForB2B.getPriceOverride().doubleValue());
							productManager.setTaxprice(dbOrderForB2B.getTaxRate().doubleValue());
							productManager.setBanben(dbOrderForB2B.getItemRevision());
							productManager.setDanwei(dbOrderForB2B.getUnitOfMeasure());
							productManager.setPaymentDate(dbOrderForB2B.getPromiseDate());
							productManager.setOrderManager(outOrderManager);
							// 查询价格信息
							String hql_price = "from Price where partNumber=? and pricePeriodStart<'"
									+ Util.getDateTime() + "' and (pricePeriodEnd>'" + Util.getDateTime()
									+ "' or pricePeriodEnd is null or pricePeriodEnd='') ";
	 ...
	 */
	@Override
	public String updateOrderForB2B(OrderForB2B pageOrderForB2B) throws IOException {
		OrderForB2B dbOrderForB2B = findPOById(pageOrderForB2B);
		if (dbOrderForB2B != null) {
			...
			if ("1".equals(pageOrderForB2B.getTypeLookupCode())) {
				quantityOrdate = "\"needQuantity\":\"" + pageOrderForB2B.getQuantity().intValue() + "\",";
			} else if ("2".equals(pageOrderForB2B.getTypeLookupCode())) {
				quantityOrdate = "\"promiseDateStr\":\"" + pageOrderForB2B.getPromiseDate() + "\",";
			}
			sb.append("{\"instanceId\":\"" + dbOrderForB2B.getInstanceId() + "\",\"lineLocationId\":\""
					+ dbOrderForB2B.getLineLocationId() + "\"," + quantityOrdate + "\"promiseDateChangeReason\":\""
					+ pageOrderForB2B.getRemark() + "\",\"typeLookupCode\":\"" + pageOrderForB2B.getTypeLookupCode()
					+ "\"}");
			OutputStream output = connection.getOutputStream();
			output.write(sb.toString().getBytes("UTF-8"));
			output.flush();
