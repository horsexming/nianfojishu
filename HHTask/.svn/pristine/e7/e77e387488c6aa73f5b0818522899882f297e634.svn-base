			...
			<div align="center">
				...
				<form id="sonform" method="post">
					...
					<table class="table">
						...
						<tr>
							<s:iterator value="procardTemplatesbList" id="pageProcardTem"
								status="pageindex">
								...
								<s:else>
									<td align="center" style="width: 100px;">
										<s:if test="#pageProcardTem.procardStyle=='外购'">
											<input
												name="procardTemplatesbList[<s:property value="#pageindex.index" />].quanzi1"
												value="<s:property value="#pageProcardTem.quanzi1" />"
												style="width: 30px;">
								:
								<input
												name="procardTemplatesbList[<s:property value="#pageindex.index" />].quanzi2"
								...
								:
								<input
												name="procardTemplatesbList[<s:property value="#pageindex.index" />].corrCount"
									...
									<td align="center" style="width: 100px;">
										<SELECT
											name="procardTemplatesbList[<s:property value="#pageindex.index" />].lingliaostatus">
											<s:if test="#pageProcardTem.lingliaostatus=='否'.toString()">
												<option>
													否
												</option>
								...
								<td align="center">
									<input type="hidden"
										name="procardTemplatesbList[<s:property value="#pageindex.index" />].id"
										value="<s:property value="#pageProcardTem.id" />">
									<input
										name="procardTemplatesbList[<s:property value="#pageindex.index" />].xuhao"
...
function sonMoveStatus() {
	...
	$
			.ajax( {
				type : "POST",
				url : "procardTemplateSbAction_sonMoveStatus.action",
				dataType : "json",
				...
				window.location.href = "procardTemplateSbAction_moveStatus.action?id=${procardTemplatesb.id}";
			}
			});
}
function deleteSons() {
	...
	$
			.ajax( {
				type : "POST",
				url : "procardTemplateSbAction_deleteSons.action",
				dataType : "json",
				data : $("#sonform").serialize(),
				success : function(msg) {
					alert(msg);
