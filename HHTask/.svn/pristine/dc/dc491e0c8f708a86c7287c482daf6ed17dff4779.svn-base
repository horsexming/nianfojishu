...
public class InDoorScreenServerImpl implements InDoorScreenServer{
	...
	public Map<String, Object> findScreenInfoByScreenId(Integer id){
		...
		String dateTime1 = Util.getDateTime();
...
//				users.setBacklogList(backlogList);
			...
			//--------------快递已存入信息
			List<ResAccess> resList1 = new ArrayList<ResAccess>();
			String cfsjString="";
			...
			for (ResAccess resAccess : resList) {
				...
				try {
					Long cfsj = Util.getDateDiff(resAccess.getCopenTime(),"yyyy-MM-dd HH:mm:ss",dateTime1,"yyyy-MM-dd HH:mm:ss");
					cfsjString = Long.toString(cfsj/3600);
				} catch (ParseException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				resAccess.setPicture(picture);
				resAccess.setCopenTime(cfsjString);
			}
			map.put("res", resList);
