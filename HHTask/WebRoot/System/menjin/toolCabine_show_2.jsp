				...
				<table class="table">
					...
					<s:iterator value="toolCabineList" id="toolCabines"
						...
						<td align="center" colspan="1">
							<a href="ToolCabineAction_toupdate.action?toolCabine.id=${toolCabines.id}&tag=${tag}">修改</a>
							<s:if test="pageStatus=='admin'">
								<a onclick="return window.confirm('您将删除数据，是否继续?')"
									href="ToolCabineAction_delete.action?id=${toolCabines.id}&tag=${tag}&pageStatus=${pageStatus}">/删除</a>
							</s:if>
							<a href="ToolCabineAction_findUsersById.action?toolCabine.id=${toolCabines.id}&tag=${tag}">/绑定用户</a>
						</td>
						<td align="center">
							<s:iterator value="toolCabines.usersList"/>
							${toolCabines.usersList}
						</td>
					</s:iterator>
					<tr>
