			...
			<form method="post" id="form" enctype="multipart/form-data">
				<table class="table" style="width: 100%;">
					<s:if test='pageStatus!="htda"'>
						<tr>
							...
							<td colspan="3">
								...
								<%--							名章--%>
								<input type="checkbox" value="公章" id="sealName"
									name="sealLog.name" />
								公章
					...
					<tr id="needSave4">
						...
						<td>
							<input type="text" class="Wdate" name="sealLog.sxtime"
								id="retime"
								onClick="WdatePicker({dateFmt:'yyyy-MM-dd',skin:'whyGreen'})" />
						</td>
					</tr>
...
function tosubmit() {
	...
	for ( var i = 0; i < sealNames.length; i++) {
		if (sealNames[i].checked == true) {
			...
			if ('机密文件' == sealNames[i].value) {
				var textValue = $("#sealName1").val();
	...
	if ('${pageStatus}' != 'htda' && (sealName == null || sealName == "")) {
		alert("请选择印章名称");
		return;
						...
						function() {
							$(
								"<option value='"
									...
									+ this
									+ "</option>")
								.appendTo("#documentType")
						}
					);
					$("<option value='其他'>其他</option>").appendTo("#documentType");
				}
			},
			error : function() {
				alert("服务器异常!");
