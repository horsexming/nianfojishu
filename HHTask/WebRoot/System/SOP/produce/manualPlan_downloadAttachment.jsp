	...
	<body>
		...
		<div id="gongneng" style="width: 100%;">
			<div align="center">
				...
				<form action="ManualOrderPlanAction_findmanualPlanList.action"
					...
					<table class="table">
						...
						<tr>
							...
							<td>
								<input name="startTime" class="Wdate" id="startTime" value="${startTime }"
							...
							<td>
								<input name="endTime" class="Wdate" id="endTime" value="${endTime }"
									onClick="WdatePicker({dateFmt:'yyyy-MM-dd',skin:'whyGreen'})" />
							</td>
						</tr>
					</table>
				...
				<form id="fromDownload" action="ManualOrderPlanAction_downLoadMultFile.action" method="post">
				<div align="right" style="padding-right: 10px;">
					<input type="button" value="下载附件" class="input" onclick="javascript:downloadAttrment();">
				</div>
					<table class="table">
						<tr align="center" bgcolor="#c0dcf2" height="50px">
							...
							<s:if test='tag=="download"'>
								<th>
									
									<input type="checkbox" onclick="chageAllCheck(this)">全选<br>
								</th>
							</s:if>
						</tr>
						<s:iterator value="mod1List" id="pageList" status="pagestatus">
							...
							<s:if test='tag=="download"'>
								<td>
									<input name="mod1List[${pagestatus.index }].markId" type="hidden" 
										value="${pageList.markId}" >
									<input name="mod1List[${pagestatus.index }].fileUrl" onclick="chageNum(this)"
										type="checkbox" value="${pageList.fileUrl}" class="checkboxFile"/>
									<input type="hidden" value="${pageList.fileName }" 
...
function toShowWW(status){
	window.location.href="ManualOrderPlanAction_findmanualPlanList.action?status="+status+"&flag=${flag}&my=${my}&tag=${tag}";
}
...
function downloadAttrment(){
	/*var bool = true;
	var arr= new Array();
	var selecteds = $(".checkboxFile");
	if(selecteds!=null && selecteds.length>0){
		for(var i=0;i<selecteds.length;i++){
			if(selecteds[i].checked == true){
				arr.push(selecteds[i].value);
				bool = false;
				break;
			}
		}
	...
	/*var form = new FormData(document.getElementById("fromDownload"));
      $.ajax({
          url:"${pageContext.request.contextPath}/ManualOrderPlanAction_downLoadMultFile.action",
          type:"post",
          data:form,
          processData:false,
          contentType:false,
