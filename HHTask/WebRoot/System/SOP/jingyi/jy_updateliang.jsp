			...
			<form action="procardTemplateJYAction_updatejgljwr.action" method="post" onsubmit="return validateFrom();">
				<table>
					<tr>
						...
						<td>
							<textarea id="updateContext" name="updateContext" rows="5" cols="40"><s:property value="processTemplateJY.contextLiang"/></textarea>
						</td>
					</tr>
					<tr>
						<th>
						...
						<td>
						<input type="hidden" value="liang" name="pageStatus">
				...
				<s:else>
				<br/>
				改进方案：<textarea id="updateContext" name="updateContext" rows="5" cols="40"><s:property value="processTemplateJY.contextLiang"/></textarea>
				<div id="showAll" style="background-color: #ffffff; position: absolute; visibility: hidden; z-index: 1;width: 200px"></div>
							<br/>选择量具：<s:if test="processTemplateJY.measuringId!=null">
							<input type="text" id="shortname" onkeyup="getAllNames()" style="height: 20px; width: 200px" onFocus="init()"
								onBlur="hidediv()"   value="<s:property value="processTemplateJY.measuring_no"/>::<s:property value="processTemplateJY.measuringMatetag"/>" />
                        </s:if>
                        <s:else>
							<input type="text" id="shortname" onkeyup="getAllNames()" style="height: 20px; width: 200px" onFocus="init()"
								onBlur="hidediv()"   />
                        </s:else><br/>
								<input type="button" value="确定" onclick="updateliangju()"/>
				</s:else>
...
function getAllNames() {
	...
	$.ajax({
				...
				success : function(data) {
					$("#showAll").empty();
					$(data).each(function() {
										var no = $(this).attr('measuring_no').replace(shortNo,"<font color='red'>"+shortNo
																+ "</font>");
