	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
				<h3>报废历史记录管理</h3>
				<br/>
		<form action="LendNectAction!queryScrapHistory.action" method="post" onsubmit="return testTime()" name="myForm">
				<table class="table" >
					<tr>
						<th>
							部门：
						...
						<td>
							<input type="text" name="scrapHistory.dept"
								value="${scrapHistory.dept}" />
						</td>
						<th>
							卡号：
						</th>
						<td>
							<input type="text" name="scrapHistory.cardNum"
								value="${scrapHistory.cardNum }" />
						</td>
						<th>
							姓名：
						</th>
						<td>
							<input type="text" name="scrapHistory.username"
								value="${scrapHistory.username}" />
						</td>
					</tr>
					<tr>
						<th>
						...
						<td>
							<input type="text" name="scrapHistory.goodsMarkId"
								value="${scrapHistory.goodsMarkId}" />
						</td>
						<th>
							批次：
						</th>
						<td>
							<input type="text" name="scrapHistory.goodsLotId"
								value="${scrapHistory.goodsLotId}" />
						</td>
						...
						<td>
							<input type="text" name="scrapHistory.goodsFullName" value="${scrapHistory.goodsFullName }"/>
						</td>
<%--						<th>--%>
<%--							规格：--%>
<%--						</th>--%>
<%--						<td>--%>
<%--							<input type="text" name="scrapHistory.format" value="${scrapHistory.format}"/>--%>
<%--						</td>--%>
			...
			<div style="color: red;font-size: 14px;" id="message">
			
			</div>	
			
			<form action="LendNectAction!exportScrapHis.action"	 method="post" onsubmit="return checkPrint()" name="myForm1">			
				<table class="table">
...
<%--						<td><input type="checkbox" name="selectedScrap" class="qx" value="${pageList.id}"/></td>--%>
<%--						--%>
						<td>
							<s:if test="#pageStatus.index%2==1">
								<font>
							...
							<s:property value="#pageStatus.index+1" />
									...
									<td>
										<a href="javascript:void(0)" onclick="delScrap(${pageList.id})">删除</a>
									</td>
						</s:iterator>
...
<%--				</tr>--%>
		...
		<script type="text/javascript">
			...
			function delScrap(scrapId){
				var flag=confirm("确定删除吗?");
				if(flag){
					$.ajax( {
							type : "POST",
							url : "LendNectAction!deleteOneScrapHistory.action",
							data : {
								scrapId:scrapId   
							},
							dataType : "json",
							...
							success : function(data) {
								if(data){
									...
									window.location.href="LendNectAction!queryScrapHistory.action?message="+data;
									$("#message").html("删除成功");
								}else{
									$("#message").html("删除失败");
								}
...
<%--								}--%>
			...
			function exportExcel(obj){
				$(obj).attr("action","LendNectAction!exportScrapHis.action");
				$(obj).submit();
				$(obj).attr("action","LendNectAction!queryScrapHistory.action");
			}
		...
		function checkPrint(){
			var selectEd=document.getElementsByName("selectedScrap");
		...
		$(function(){
			...
			$("#textselectgoodHouseName").val('${nect.goodHouse}')
