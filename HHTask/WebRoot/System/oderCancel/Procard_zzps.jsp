		...
		<div id="gongneng" style="width: 100%;" >
			<div align="center">
				...
				<select  id="type" style="width:80px;">
					<option></option>
					<option value="zz">自制</option>
					<option value="ww">委外</option>
				</select>
				<b>工序名:</b>
				<select  id="processName" name="processName">
					<option></option>
				</select>
				<input type="button" value="查询" onclick="selectProess()" style="width: 80px;height: 40px;"/>
				<br />
				<table class="table" id="mytable">
				</table>
				<div id="show" style="color:red;font-size: x-large; ">
					<br/>
					正在加载,请稍候<span id="showMess1_font"></span>
				</div>
			</div>
		</div>
					...
					if (data != null && data.length > 0) {
						...
						for ( var i = 0; i < data.length; i++) {
							...
							$("#mytable")
									.append(
											'<tr id=tr_'
													...
													+ procard.markId
													+ '</a></td>'
													+ '<td style="width: 70px;">'+procard.status+'</td><td style="width: 100px;">'
													+ procard.selfCard
													+ '</td>'
							...
							getProcessInfor(procard.id, procard.id,
									procard.filnalCount,procard.jihuoStatua,type,workShop);
						}
					}
				}
			})
}
function getProcessInfor(procardId, num, num1,jihuoStatua,type,workShop) {
	$
			.ajax( {
				type : "POST",
				...
				success : function(data) {
					if (data != null && data.length > 0) {
						...
						for ( var i = 0; i < data.length; i++) {
							...
							if(agreeWwCount>0){
								pagestatus = "<font color='#08088A'>外委</font>(<font color='red'>"+agreeWwCount+"</font>)"
							}
							var num3=	(num2/num1).toFixed(2)*50;
							var jiduti ='<div style="width: 50px;height: 10px;border: 1px solid #33CC33;">'
										+'<div style="width: '+num3+'px;height: 10px;background-color: #33CC33; float: left;">'
										+'<font size="1px;" color="#000">'+num2+'/'+num1+'</font>'
										'</div>'
										+'</div>';
							tr1 += '<td style="font-size:10px;">' + process[0] + '-' + process[1]
									+ '<hr><div align="right">'
									+jiduti+'</div></td>';
							tr2 += "<td ><a style='cursor:pointer;' href='javascript:;' onclick='showprocesslog("+process[4]+")'><div id='jdt" + num + "_" + i
									+ "' class= 'jindu' data=" + (num2 / num1)
									* 100 + " ></div></a></td>";
