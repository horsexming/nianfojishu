						...
						<tr>
							...
							<td>
								<input id="markIds" type="text"
									name="breaksubmit.markId" style="width: 110px;"
									value="${breaksubmit.markId}" />
					...
					<s:if test="{bsList.size()>0}">
						<s:iterator value="bsList" status="se" id="pagebreak">
							...
							<tr>
								<th rowspan="3">
									<div
										onclick="getcheckList2(${pagebreak.processId},'${pagebreak.id}' )"
							...
							<tr>
								<th>
									<a href="javascript:;"
										onclick="toFindForm('markIds','${pagebreak.markId}')">${pagebreak.markId} </a>
								</th>
								<td>
									${pagebreak.proName}
								</td>
							...
							<tr>
								<th align="left">
									[第${pagebreak.processNo}工序]${pagebreak.processName}
								</th>
<%--								<td><fmt:formatNumber value="${pageprocess.applyCount-pageprocess.submmitCount}" pattern="#" />--%>
<%--									PSC--%>
<%--								</td>--%>
								<td>
									<a href="javascript:;"
...
function getcheckList2(id,id2) {
	window.location.href = "InsRecord_getXjProcess.action?id=" + id
			+ "&cpage=${cpage}&status=sqjy&breakSubmit.id="+id2+"&tag=${pageStatus}";
}
