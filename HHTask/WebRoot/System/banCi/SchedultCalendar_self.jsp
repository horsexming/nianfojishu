	...
	$("#reserveinfo").hide();
				...
				var calendar = $('#calendar').fullCalendar({
					...
					events : function(start, end, callback) {
						var usersIds = "${param.usersId}";
						var usersId;
						if(usersIds!=null && usersIds.length>0){
							usersId = usersIds.split(",")[0];
						}else{
							usersId = "${sessionScope.Users.id}";
						}
						$.ajax( {
							...
							success : function(doc) {
								...
								$(doc).each(function() {
									...
									var banciName = $(this).attr("banci_name");
									if(banciName!=null && banciName !="" && banciName!="null"){
										title+="\n"+banciName;
									}
					...
					eventClick : function(event) {
						if("${param.usersId}"==null || "${param.usersId}"==""){
							return false;
						}
						...
						$("#reserveinfo").dialog({
							...
							buttons : {
								...
								"修改" : function() {
									...
									$.ajax({
						                url:"${pageContext.request.contextPath}/banCiAction_updateSchedulByUsersBanci.action",
						                type:"post",
						                data:form,
						                processData:false,
						                contentType:false,
...
// 												window.location.reload();
												$("#reserveinfo").remove();
												parent.loadUsersCalendar('${param.usersId}');
												return true;
						                	}
						                },error:function(XMLHttpRequest, textStatus, errorThrown){
											alert("系统异常");
						...
						$.ajax({
							type:"post",
							url:"${pageContext.request.contextPath}/banCiAction_findAllBanCi.action",
							dataType:"json",
							success:function(data){
								$("#banCiId").empty();
								$("#banCiId").append("<option ></option>");
								$(data).each(function(){
									$("#banCiId").append("<option value='"+this.id+"'>"+this.name+"</option>");
								});
							},error:function(XMLHttpRequest, textStatus, errorThrown){
								alert("系统异常");
			                }
						...
						var dateTime = $.fullCalendar.formatDate(event.start,"yyyy-MM-dd")
...
.fc-event-inner{
					...
					<DIV id="reserveinfo">
						<form action="" method="post" id="updateSchedule">
							...
							<div style='margin-top:10px;' align='left'>
								...
								<input type="hidden" id="dateTime" name="schedulingTable.dateTime" >
								<input type="hidden" class="" id="usersIds" name="usersIds" value="${param.usersId }">
							</div>
							<div style='margin-top:10px;' align='left'>
								<b>班次:</b>
								<select name="schedulingTable.banCi.id" id="banCiId" class="form-control" onchange="changeBanCi()">
	...
	<script type="text/javascript">
		function changeBanCi(){
			var banCiId = $("#banCiId").val();
