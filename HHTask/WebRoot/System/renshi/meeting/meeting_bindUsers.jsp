			...
			<form id="sendform" 
				...
				<div align="center" style="width:30%;float:left;">
					...
					<table  id="alreadyBinder">
						<s:iterator value="personList" id="list" >
							<tr id="tr_${list.userId}">
								<td>
									<input type='checkbox' name='payersName' value='${list.userId}' />
									${list.userName}
								</td>
							</tr>
						</s:iterator>
...
function addPlayers(){
	...
	$("input[name='receiver']:checked").each(function(){ 
		...
		if(flag){
			$.ajax({
				type:"post",
				url:"meetingAction!bindMeetingUsers.action",
				dataType:'json',
				cache:false, 
	       		async:false, 
				data:{
				...
				success:function(data){
					if("success"==data){
						...
						"<input type='checkbox' name='payersName' value='"+userId+"' />"+userName+"</td></tr>");
...
function cancelPlayer(){
	...
	$("input[name='payersName']:checked").each(function(){ 
		...
		if(flag){
			$.ajax({
				type:"post",
				url:"meetingAction!cancelMeetingUsers.action",
				dataType:'json',
				cache:false, 
	       		async:false, 
				data:{
