		...
		<div id="gongneng"
			...
			<div align="center">
				...
				<table style="width: 980px; border-collapse: collapse;font-size: 12px" border="0"
					...
					<s:iterator id="overtime" value="overtimeList" status="ststusOvertime">
						<s:if test="#ststusOvertime.first">
							<tr bgcolor="green">
								<th colspan="11" align="center">
									<font color="#ffffff"> 加班记录</font>
								</th>
							</tr>
						</s:if>
						...
						<td>
							<a target="_blank"
								href="overtimeAction!getUpdatePage.action?id=${id}">
								修改</a>/
							<a target="_blank"
								href="overtimeAction!updateOvertime.action?overtime.id=${id}&overtime.status=通过">
								通过</a>/
							<a target="_blank"
								href="overtimeAction!updateOvertime.action?overtime.id=${id}&overtime.status=不通过">
								不通过</a>/
							<a onclick="return window.confirm('确认要删除该加班记录?')"
								href="overtimeAction!deleteOvertime.action?id=${id}">删除</a>
						</td>
						</tr>
					</s:iterator>
