...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<form action="overtimeAction!backupOvertime.action" method="post"  onsubmit="return checkType();" role="form">
				<div class="row">
					<div class="col-xs-12 text-center"><h2>加班后补</h2></div>
				</div>
				...
				<div class="row">
					<label for="name" class="col-xs-12">打卡时间列表 </label><br>
					<div class="col-xs-12 row" >
						<s:iterator value="attendanceTowList" id="st" status="status">
							<div class="col-xs-2">
								&nbsp;&nbsp;<input readonly="readonly" title="点击复制" value="${st.addTime}" class="form-control"/>&nbsp;&nbsp;
							</div>
						</s:iterator>
					</div>
				</div>						
							...
							<tbody>
								<s:iterator value="overtimeDetailList" id="otd" status="ps">
									<tr id="tr_${ps.index}">
					        			...
					        			<td>
					        				<input type="hidden" name="overtimeDetailList[${ps.index}].id" value="${otd.id}" id="detailId_${ps.index}" >
					...
					<div class="col-xs-12">
						<div class="form-group">
							...
							<div class="content row col-xs-12">
								<textarea class="form-control" rows="3" id="overtimeNeirong" 
									name="overtime.overtimeNeirong">${overtime.overtimeNeirong}</textarea>
							</div>
						</div>
					</div>
				</div>
...
<%-- 										<input type="radio" name="overtime.huanxiu" value="${hxStatus }"/>${hxStatus } --%>
			...
			<textarea id="overtimeInfo" style="display: none;">
				<tr id="tr_0">
        			...
        			<td>
        				<input type="hidden" name="overtimeDetailList[0].id" value="${otd.id}" id="detailId_0" >
        				<button class='btn btn-default' onclick='delDetail(this,"_0")' id="del_0">删除</button>
        			</td>
        		</tr>
			</textarea>
			<!-- 拼接加班信息 over-->
...
$jq(function() {
	...
	$("input[title='点击复制']").click(function () {
		//$(this).focus();
		$(this).select();
//         var Input = $('.addresNm');
//         Input.focus()
...
// 	$("#joined1").val(joined1);
// 	$("#joined2").val(joined2);这里更改overtime的开始时间和结束时间
	if(!flag){
		alert("请填写加班记录");
		return false;
	}
...
function computeDate(_index){
	...
	//后补功能需要校验打卡时间列表
	var attList = $("input[title='点击复制']");
	if(recordStart!=start){
		var attStartFlag = false;
		for(var i=0;i<attList.length;i++){
			var attendance = attList[i].value;
	...
	var detailId = $("#detailId"+_index).val();
	$("tbody").append(overtimeInfo);
	//$("#tr"+_index).remove();
	delDetail($("#del"+_index),_index);
	$("#detailId_"+totalIndex).val(detailId);
	$("#type_"+totalIndex).val(category);
	$("#start_"+totalIndex).val(formatTime(startDate));
	$("#end_"+totalIndex).val(end);
	$("#xiuxi_"+totalIndex).val(xiuxi);
