					...
					<div>
						...
						<div id="module1_1" align="center">
							...
							<input type="hidden" value="1" name="user.backStage">
								<select id="deptname" style="width: 100px;" onchange="changeDept();">
										<option value="0">
											请选择部门
										</option>
								</select>
							</td>
							<td align="center" id="deptstd">
							<input type="hidden" value="0" id="deptIds" name="deptIds">
								<select id="username" name="user.id" style="width: 100px;" onchange="showSubDept()">
										<option value="0">
											请先选择部门
										</option>
								</select>
...
function showSubDept() {
	...
	$.ajax( {
		type : 'post',
		url : 'SubjectBudgetAction!findAllSubDpetjsp.action',
		data : {
				id : $("#username").val()
							},
		dataType : 'json',
...
function beforeCheck(treeId, treeNode) {
	$.ajax( {
		url : 'SubjectBudgetAction!bdAndqxUser.action',
		dataType : 'json',
		data : {
				id : $("#username").val(),
				'subBudgetRate.id' :treeNode.id
		},
		cache : false,//防止数据缓存
		success : function(data) {
			return data;
