...
<html>
	...
	<body>
		...
		<div id="gongneng"
			...
			<div align="center">
				<form action="BaoXiaoDanAction!findBaoXiaoDan1.action?tag=${tag}" method="post">
					<table class="table">
						<tr>
							<th>
								收款单位(个人) 
							...
							<th>
								<input type="text" name="baoxiao.shoukuanRen" value="${baoxiao.shoukuanRen}" />
							</th>
							<th>
								付款方式
								</th>
							<th>
								...
								<option value="${baoxiao.payStyle}">${baoxiao.payStyle}</option>
								<option value="">
										无
									</option>
								<option value="现金">
							...
							<th>
								<input type="text" name="baoxiao.baoxiaoren"  value="${baoxiao.baoxiaoren}" />
							</th>
						...
						<tr>
							<th>申报编号：</th>
							<th>
							<input type="text" name="baoxiao.baoxiaoBarcode"  value="${baoxiao.baoxiaoBarcode}" />
							</th>
							<th>
								日期从：
								</th>
						...
						<th>部门</th>
						<th>
						<select style="width:150px;" name="baoxiao.dept" id="dept" onclick="createDept('dept')" >
						<option value="${baoxiao.dept}">${baoxiao.dept}</option>
				...
				<table class="table">
					...
					<s:if test="{list.size()>0}">
						<s:iterator value="list" status="se" id="bxd">
							...
							<td>
								<a href="BaoXiaoDanAction!getBaoXiaoDan1.action?id=${id}&tag1=detail&tag=${tag}">查看</a>&nbsp;&nbsp;&nbsp;
								<a href="BaoXiaoDanAction!getBaoXiaoDan1.action?id=${id}&tag1=update&tag=${tag}">修改</a>&nbsp;&nbsp;&nbsp;

								 <s:if test="tag=='manger'">
								 <s:if test="%{#bxd.dealstatus==null||#bxd.dealstatus==''}">
								 	<a href="BaoXiaoDanAction!updateXiaoDan1.action?id=${id}&tag=${tag}">确认</a>
								 </s:if>
								 </s:if>
								 <!-- 
		...
		function exportExcel(objForm) {	
			objForm.action="BaoXiaoDanAction!exportEXCEL1.action?tag=bxd";
			objForm.submit();	
		}
		</script>
	</body>
