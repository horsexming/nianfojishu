			...
			$(allUnit).each(
					function() {
						if(this.unitname === unit){
							$(
								"<option value='" + this.unitname + "' selected='selected'>"
...
var selectname = "";
var pageId =0;
function duoxuaSelect(id,value,hidden,text){//id-username
	if(pageId != id){
		selectname = "";
	}
	$("#"+id).hide();
	...
	var hidtext = 'text';
	if(hidden == 'hidden'){
		hidtext = '<input type="text" id="showText'+id+'"  value="'+text+'">';
	}
	var str = '<div class="duoxuaselect_div"><div style="float: left;">' +hidtext+
	'<input type="'+hidden+'" id="textselect'+id+'" name='+selectname+' value="'+(value==undefined?"":value)+'" />' +
	'</div><div style="float: left;margin-left: 4px;" class="select_checkBox"><div class="chartQuota"><p>' +
	...
	if (chexkSelects != null && chexkSelects.length > 0) {
		for ( var i = 0; i < chexkSelects.length; i++) {
			if (chexkSelects[i].checked) {
				...
				var parent =$(chexkSelects[i]).parent();
