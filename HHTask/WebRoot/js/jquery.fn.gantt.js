...
(function($) {//(function(arg){...})(param) 杩欏氨鐩稿綋浜庡畾涔変簡涓�釜鍙傛暟涓篴rg鐨勫尶鍚嶅嚱鏁帮紝骞朵笖灏唒aram浣滀负鍙傛暟鏉ヨ皟鐢ㄨ繖涓尶鍚嶅嚱鏁�
	"use strict";
	$.fn.gantt = function(options) {
		var cookieKey = "jquery.fn.gantt";
		var scales = [ "hours", "days", "weeks", "months" ];
		var settings = {//閮ㄤ欢璁剧疆闆�
			source : null,
			itemsPerPage : 7,
		...
		$
				.extend($.expr[":"], {//鑷畾涔夐�鎷╁櫒鏂规硶锛坅锛氬厓绱狅紝i锛氱储寮曪紝m锛氬�锛�
							findday : function(a, i, m) {//浠ユ绉掍负鏃堕棿鍗曚綅鍖归厤涓�釜鎸囧畾鐨勬棩鏈�
		...
		$.extend($.expr[":"], {
			findweek : function(a, i, m) {//浠ユ绉掍负鏃堕棿鍗曚綅鍖归厤涓�釜鎸囧畾鐨勫懆
					var cd = new Date(parseInt(m[3], 10));
					var id = $(a).attr("id");
					id = id ? id : "";
					var si = id.indexOf("-") + 1;
		...
		$.extend($.expr[":"], {
			findmonth : function(a, i, m) {//锛氫互姣涓烘椂闂村崟浣嶅尮閰嶄竴涓寚瀹氱殑鏈�
					var cd = new Date(parseInt(m[3], 10));
		...
		Date.prototype.getWeekId = function() {//鑾峰彇weekid鐢ㄤ簬鏍囪瘑week鐨刣iv鐨刬d鐗规�锛岃繑鍥炲瓧绗︿覆锛屾牸寮忎负dh-YYYY-WW锛屽叾涓殑ww鏄竴骞翠腑鐨勭鍑犲懆
		...
		Date.prototype.genRepDate = function() {//鎸夐儴浠惰缃睘鎬т腑scale鐨勫�鑾峰彇鏃堕棿鑼冨洿锛屼互绉掍负鏃堕棿鍗曚綅
		 ...
		 */
		var core = {// Grid绠＄悊鍣紙璐熻矗瀵艰埅鍜屾覆鏌擄級锛歝ore

			elementFromPoint : function(x, y) {//鑾峰彇浣嶄簬鎸囧畾鐐圭殑鏈�珮澶勭殑鍏冪礌
			...
			init : function(element) {//鍒濆鍖栬鍥撅紝璁＄畻琛屾暟銆侀〉鏁般�鍙鐨勫紑濮嬫椂闂翠笌缁撴潫鏃堕棿
				element.rowsNum = element.data.length;//琛屾暟
				element.pageCount = Math.ceil(element.rowsNum
						/ settings.itemsPerPage);//椤垫暟
				
				
				
				element.rowsOnLastPage = element.rowsNum
						- (Math.floor(element.rowsNum / settings.itemsPerPage) * settings.itemsPerPage);//鏈�悗涓�〉鏉℃暟
			...
			render : function(element) {//娓叉煋grida
				...
				var $leftPanel = core.leftPanel(element);//鍒涘缓宸︿晶Panel
				content.append($leftPanel);
				var $rightPanel = core.rightPanel(element, $leftPanel);//鍒涘缓鍙充晶澶撮儴Panel
				content.append($rightPanel);
				var mLeft, hPos;
				content.append(core.navigation(element));//瀵艰埅鏉�
				...
				core.markNow(element);//绉婚櫎鈥漺d鈥渃lass娣诲姞鈥漷oday鈥渃lass鍒板綋鍓嶇殑scale妯″紡
				core.fillData(element, $dataPanel, $leftPanel);//濉厖鍥捐〃锛岃В鏋愭暟鎹苟濉厖鍒皃anel
				...
				if (settings.scrollToToday) {//璁剧疆鏄惁婊氬姩鍒颁粖澶�
					var startPos = Math
			...
			leftPanel : function(element) {
				...
				ganttLeftPanel.append(entries.join(""));//鏁扮粍鍚�join("");join() 鏂规硶鐢ㄤ簬鎶婃暟缁勪腑鐨勬墍鏈夊厓绱犳斁鍏ヤ竴涓瓧绗︿覆銆傚厓绱犳槸閫氳繃鎸囧畾鐨勫垎闅旂杩涜鍒嗛殧鐨勩�   
			...
			dataPanel : function(element, width) {
				...
				/*
				 * Mouse wheel events婊氳疆浜嬩欢
				 */
				var mousewheelevt = (/Firefox/i.test(navigator.userAgent)) ? "DOMMouseScroll"
						: "mousewheel";////濡傛灉鏄伀鐙愭祻瑙堝櫒灏眒ousewheelevt=DOMMouseScroll;鍚﹀垯mousewheelevt=mousewheel,鍥犱负鐏嫄缁戝畾婊氳疆浜嬩欢鏄疍OMMouseScroll

				if (document.attachEvent) {//IE涓垜浠彲浠ヤ娇鐢╝ttachEvent
					element.attachEvent("on" + mousewheelevt, function(e) {
						core.wheelScroll(element, e);
					});
				} else if (document.addEventListener) {//鍙儨杩欎釜寰蒋鐨勭浜烘柟娉�鐏嫄鍜屽叾浠栨祻瑙堝櫒閮戒笉鏀寔,骞歌繍鐨勬槸浠栦滑閮芥敮鎸乄3C鏍囧噯鐨刟ddEventListener鏂规硶
				...
				dataPanel.click(function(e) {
					e.stopPropagation();//涓嶅啀娲惧彂浜嬩欢銆傜粓姝簨浠跺湪浼犳挱杩囩▼鐨勬崟鑾枫�鐩爣澶勭悊鎴栬捣娉￠樁娈佃繘涓�浼犳挱銆傝皟鐢ㄨ鏂规硶鍚庯紝璇ヨ妭鐐逛笂澶勭悊璇ヤ簨浠剁殑澶勭悊绋嬪簭灏嗚璋冪敤锛屼簨浠朵笉鍐嶈鍒嗘淳鍒板叾浠栬妭鐐广�
						var corrX, corrY;
			...
			rightPanel : function(element, leftPanel) {//鍒涘缓鍙充晶澶撮儴Panel
				...
				switch (settings.scale) {//鐢樼壒鍥炬瘡涓�垪鐨勬椂闂磋寖鍥�
				// hours ///////////////////////////////////////////////////////////////////////////////////////// 
				case "hours":
				...
				case "weeks":
					...
					for ( var i = 0; i < range.length; i++) {
						...
						//genRepDate():鎸夐儴浠惰缃睘鎬т腑scale鐨勫�鑾峰彇鏃堕棿鑼冨洿锛屼互绉掍负鏃堕棿鍗曚綅
						//getWeekId():鑾峰彇weekid鐢ㄤ簬鏍囪瘑week鐨刣iv鐨刬d鐗规�锛岃繑鍥炲瓧绗︿覆锛屾牸寮忎负dh-YYYY-WW锛屽叾涓殑ww鏄竴骞翠腑鐨勭鍑犲懆
						//getWeekOfYear():鑾峰彇鏃ユ湡鍦ㄤ竴骞翠腑鐨勭鍑犲懆
					}
			...
			navigation : function(element) {//瀵艰埅
			...
			createProgressBar : function(days, cls, desc, label, dataObj) {//鍒涘缓杩涘害鏉�    
			...
			markNow : function(element) {//绉婚櫎鈥漺d鈥渃lass娣诲姞鈥漷oday鈥渃lass鍒板綋鍓嶇殑scale妯″紡
				switch (settings.scale) {
				case "weeks":
			...
			fillData : function(element, datapanel, leftpanel) {//濉厖鍥捐〃锛岃В鏋愭暟鎹苟濉厖鍒皃anel
				...
				$
						.each(
								element.data,
								function(i, entry) {//閬嶅巻澶勭悊element.data锛屽彲浠ユ槸鏁扮粍銆丏OM銆乯son绛夛紝鍙栧喅浜庣洿鎺ョ粰瀹氭垨鑰卆jax杩斿洖鐨勭被鍨�i鏄綋鍓嶅厓绱犵殑浣嶇疆锛宔ntry鏄�銆�
									if (i >= element.pageNum
											* settings.itemsPerPage
											&& i < (element.pageNum
													* settings.itemsPerPage + settings.itemsPerPage)) {
										//itemsPerPage鍒嗛〉鐨勬瘡椤垫暟鎹鏁�i>=0 && i<6  i>=6 && i<12
										...
										$
												.each(
														...
														function(j, day) {
															...
															case "weeks":
																var dtFrom = tools
																		.dateDeserialize(day.from);//褰撳墠椤佃繘搴︽潯璧峰鏃堕棿闆嗗悎
																var dtTo = tools
																		.dateDeserialize(day.to);
																//getDate():浠�Date 瀵硅薄杩斿洖涓�釜鏈堜腑鐨勬煇涓�ぉ (1 ~ 31)銆�
																...
																//Math.round():鍙妸涓�釜鏁板瓧鑸嶅叆涓烘渶鎺ヨ繎鐨勬暣鏁般�
																var dl = Math
																		.round((cTo - cFrom)
																				/ tools
																						.getCellSize()) + 1;//杩涘害鏉℃墍鍗犲崟鍏冩牸鏁�
																_bar = core
																		.createProgressBar(
																				dl,
																				day.customClass ? day.customClass
			...
			navigatePage : function(element, val) {//瀵艰埅鍒版寚瀹氱殑椤甸潰
			...
			navigatePage1 : function(element, val) {//瀵艰埅鍒版寚瀹氱殑椤甸潰
				core.waitToggle(element, true, function() {
					element.pageNum = val;
					element.hPosition = $(".fn-gantt .dataPanel").css(
							"margin-left").replace("px", "");
			...
			zoomInOut : function(element, val) {//鍙樻洿绌洪棿杞寸骇娆★紙zoom锛�
				core.waitToggle(element, true, function() {
					var zoomIn = (val < 0);
			...
			mouseScroll : function(element, e) {//閫氳繃榧犳爣绉诲姩鍥捐〃
				...
				core.scrollPanel(element, delta);//鏇存柊婊氬姩panel鐨刴argin
			...
			scrollPanel : function(element, delta) {//鏇存柊婊氬姩panel鐨刴argin
				if (!element.scrollNavigation.canScroll) {
					return false;
				}
			...
			synchronizeScroller : function(element) {//鍚屾婊氬姩
			...
			repositionLabel : function(element) {//閲嶆柊瀹氫綅鏁版嵁鏍囩
			...
			waitToggle : function(element, show, fn) {//鍒囨崲绛夊緟
				if (show) {
		 ...
		 */
		var tools = {//瀹炵敤宸ュ叿绫诲姛鑳�

			getMaxDate : function(element) {//杩斿洖鏈�ぇ鍙兘鐨勬棩鏈熷湪scale鍊肩殑鏍囧噯涓�
				var maxDate = null;
			...
			getMinDate : function(element) {//杩斿洖鏈�皬鍙兘鐨勬棩鏈熷湪scale鍊肩殑鏍囧噯涓�
				var minDate = null;
				$.each(element.data, function(i, entry) {
					$.each(entry.values, function(i, date) {
			...
			parseWeeksRange : function(from, to) {//杩斿洖涓�釜鏃ユ湡瀵硅薄鏁扮粍浠嬩簬from鍜宼o锛屾椂闂村崟浣嶄负鍛�
				var current = new Date(from);
				var end = new Date(to);
			...
			parseMonthsRange : function(from, to) {//杩斿洖涓�釜鏃ユ湡瀵硅薄鏁扮粍浠嬩簬from鍜宼o锛屾椂闂村崟浣嶄负鏈�
			...
			dateDeserialize : function(dateStr) {//浠庡瓧绗︿覆鍙嶅簭鍒楀寲鎴愭棩鏈�
				//         /Date()/   dateStr.replace(/\//g, " ")):鎶婃墍鏈塡鍙樻垚""  
				var date = eval("new" + dateStr.replace(/\//g, " "));
				//eval()璁＄畻鏌愪釜瀛楃涓诧紝骞舵墽琛屽叾涓殑鐨�JavaScript 浠ｇ爜銆�new Date();
			...
			getProgressBarMargin : function() {//鑾峰彇褰撳墠杩涘害鏉＄殑margin澶у皬
		...
		this.each(function() {//鐢樼壒鍥剧粍浠跺璞￠亶鍘嗗櫒锛堣礋璐ｆ墽琛屽垵濮嬪寲鎿嶄綔锛�
					//gantt div
					/**
