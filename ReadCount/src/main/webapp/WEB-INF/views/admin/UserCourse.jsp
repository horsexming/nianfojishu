                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <font size="4">统计明细</font>
                                 <span>
				                	<a href="<%=request.getContextPath()%>/Course/AdminCount">
				                		<font color = "#6CA6CD"> &nbsp; &nbsp; &nbsp;返回统计</font>
				                	</a>
				               	</span>
                            </div>
                            <div class="card-body p-0">      
                                <div class="justify-content-around mt-4 p-4 bg-light d-flex border-top d-md-down-none">
                                	<table align="center" class="table table-hover">
										<tr>	                		
					                		<td>阅读者</td>
					                		<td align="center">阅读者阅读次数</td>
					                		<td align="center">每日阅读次数参考</td>
					                		<td align="center">未完成阅读次数</td>
					                		<td>课程名</td>	             		
					                		<td>发布者</td>	                	
					                		<td>最新发布时间</td>
					                		<td>最早可阅读时间</td>
					                	</tr>
					                	<c:forEach items="${UserCour}" var="AdminCourse" varStatus="status">
					                		<tr>	                			               	               			                		                				              
					                		<td>${AdminCourse.readername}</td>
					                		<td align="center">${AdminCourse.reading}</td>
					                		<td align="center">${AdminCourse.courseRead}</td>		                			             	                		
					                		<td align="center">
					                			<c:if test="${AdminCourse.poorR>0}">${AdminCourse.poorR}</c:if>
												<c:if test="${AdminCourse.poorR<=0}">已完成</c:if>
					                		</td>
					                		...
					                		<td><fmt:formatDate value="${AdminCourse.publishTime}" pattern="yyyy-MM-dd"/></td>
					                		<td><fmt:formatDate value="${AdminCourse.buildFirst}" pattern="yyyy-MM-dd"/></td>
				                		</tr>
					                	</c:forEach>
									</table>
                                </div>
									...
									<div>					
										<div class="col-md-12 col-md-offset-6">
											<nav aria-label="Page navigation">
												<ul class="pagination">
													<li><a href="<%=request.getContextPath()%>/Admin/UserCourse/${UserCourid}?pn=1" aria-label="Previous">首页</a></li>
													<c:if test="${pageInfo.hasPreviousPage}">
														<li>&nbsp;&nbsp;
															<a href="<%=request.getContextPath()%>/Admin/UserCourse/${UserCourid}?pn=${pageInfo.pageNum-1}" aria-label="Previous">
													...
													<c:forEach items="${pageInfo.navigatepageNums}" var="page_Number">
														...
														<c:if test="${page_Number!=pageInfo.pageNum}">
															<li><a href="<%=request.getContextPath()%>/Admin/UserCourse/${UserCourid}?pn=${page_Number}">&nbsp;&nbsp;${page_Number}</a></li>
														</c:if>					  					    	
													</c:forEach>
													
													<c:if test="${pageInfo.hasNextPage}">
														<li>&nbsp;&nbsp;
															<a href="<%=request.getContextPath()%>/Admin/UserCourse/${UserCourid}?pn=${pageInfo.pageNum+1}" aria-label="Next">
																<span aria-hidden="true">&raquo;</span>
															</a>
														</li>
													</c:if>
																		    
													<li><a href="<%=request.getContextPath()%>/Admin/UserCourse/${UserCourid}?pn=${pageInfo.pages}" aria-label="Previous">&nbsp;&nbsp;尾页</a></li>							
												</ul>
											</nav>
										</div>
									<!-- 加载分页信息完毕 -->	
